...
public class LatexParserUtils {
	...
	/**
	 * Checks whether the character at position <code>index</code> is escaped by
	 * a backslash or not.
	 * 
	 * @param input
	 * @param index
	...
	/**
	 * Returns the index of the first character of the line where
	 * <code>index</code> is located. Legal line delimeters are \n, \r, \r\n.
	 * 
	 * @param input
	 * @param index
	 * @return
	 ...
	 * @param index
	 *            The index where the command occurs
	 * @return
	 * @throws BadLocationException
	 *             if index is out of bounds
	 */
	private static boolean testForCommand(String input, int commandLength, int index) {
		...
		if (index + commandLength == input.length() || !Character.isLetter(input.charAt(index + commandLength))) {

			if (!isInsideComment(input, index))
				return true;
		}
		return false;
	}
	 ...
	 * @param input
	 * @param command
	 *            The Latex command starting with a backslash (\)
	 * @param fromIndex
	 ...
	 *            The Latex command starting with a backslash (\)
	 * @param fromIndex
	 *            The index from which to backward start the search
	 * @return The position of the command, or -1 if the command is not
	 *         contained in the String
	 */
	 ...
	 * @param offset
	 * @param anchor
	 *            Must be either <code>LEFT</code> or <code>RIGHT</code>
	 * @param opening
	 * @param closing
	 *            matching character for opening
	...
	public static int findPeerChar(String input, int offset, int anchor, char opening, char closing) {
		...
		while (stack > 0) {
			...
			if (c == closing && (!isEscaped(input, index)) && (!isInsideComment(input, index)))
				stack--;
			else if (c == opening && (!isEscaped(input, index)) && (!isInsideComment(input, index)))
				stack++;
		}
		return index;
	}
	 ...
	 * @param input
	 * @param index
	 *            The index at or after the beginning of the command and before
	 *            the argument
	...
	/**
	 * Gets the command at the specified index. It returns the command if the
	 * index position is either inside the command string or inside the first
	 * mandatory argument
	 * 
	 * @param input
	 * @param index
	...
	public static IRegion getCommand(String input, int index) {
		...
		if (isInsideComment(input, pos))
			return null;
		boolean whiteSpace = false;
		if (pos > 0 && input.charAt(pos) == '}')
			pos--;
		char c = input.charAt(pos);
		while (!((pos <= 0 || c == '\\' || c == '{' || c == '}' || c == '%') && (!isEscaped(input, pos)))) {
	 ...
	 * @param input
	 * @param envName
	 *            Name of the environment, e.g. "itemize"
	 * @param beginIndex
	 *            Must be at the start or inside of \begin{env}
	 * @return The region of the \end{env} command or null if the end was not
	 *         found
	 */
	public static IRegion findMatchingEndEnvironment(String input, String envName, int beginIndex) {
		...
		do {
			...
			if (nextBegin == null) {
				...
				if (nextBegin.getOffset() > nextEnd.getOffset())
					level--;
				else
					level++;
	 ...
	 *            Name of the environment, e.g. "itemize"
	 * @param beginIndex
	 *            Must be at the start of \end{env}
	 * @return The region of the \begin{env} command or null if the end was not
	 *         found
	 */
	public static IRegion findMatchingBeginEnvironment(String input, String envName, int beginIndex) {
		...
		do {
			...
			if (nextEnd == null) {
				...
				if (nextEnd.getOffset() > nextBegin.getOffset())
					level++;
				else
					level--;
