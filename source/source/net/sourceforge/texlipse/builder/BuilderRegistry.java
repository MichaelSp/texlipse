...
/**
 * Holds a registry of available builders and runners. Implemented using the
 * Singleton pattern.
 * 
...
public class BuilderRegistry {
	 ...
	 *            output file format
	 * @param alternative
	 *            if there are more than one runner, take the
	 *            <code>alernative</code>'th runner
	 * @return a program runner capable of converting from the given input
	 *         format to the given output format
	 */
	...
	/**
	 * Hidden constructor. Creates the shared instances of the program runners
	 * and the builders.
	 * 
	 ...
	 * 
	 * Also: - we need all the runners when constructing builders - we need the
	 * number of runners when initializing preferences
	 */
	protected BuilderRegistry() {
		runnerList = new ProgramRunner[] { new LatexRunner(), new PslatexRunner(), new PdflatexRunner(),
				new XelatexRunner(), new LualatexRunner(), new BibtexRunner(), new BiberRunner(),
				new MakeindexRunner(), new DvipsRunner(), new DvipdfRunner(), new Ps2pdfRunner(),
				new MakeindexNomenclRunner(), new KpsewhichRunner() };
	}

	/**
	 * Initializes builders. This cannot be done in constructor, because
	 * builders need to use BuilderRegistry to resolve runners.
	 */
	 ...
	 * 
	 * @param builderClass
	 *            the implementing class of the builder
	 * @param outputFormat
	 *            the output format of the builder
	...
	protected Builder getBuilder(Class<? extends Builder> builderClass, String outputFormat) {
		...
		for (int i = 0; i < builderList.length; i++) {
			if (builderList[i] != null && builderList[i].getOutputFormat().equals(outputFormat)
	...
	protected Builder[] getAllBuilders(String format) {
		...
		for (int i = 0; i < builderList.length; i++) {
			if (builderList[i] != null && builderList[i].getOutputFormat().equals(format)) {
	 ...
	 * 
	 * @param i
	 *            the index number of the runner
	 ...
	 * 
	 * @param in
	 *            input format of the runner
	 * @param out
	 *            output format of the runner
	 * @return the program runner
	 */
