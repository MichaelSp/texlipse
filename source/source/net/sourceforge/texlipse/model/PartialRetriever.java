 ...
 * 
 * This class provides both a linear search algorithm (for a reference to test
 * new algorithms with) as well as a binary search algorithm.
 * 
 * @author Oskar Ojala
...
public abstract class PartialRetriever {
	...
	/**
	 * Returns (if exist) the position of the entry with the given name in the
	 * array
	 * 
	 * @param entryname
	 *            Name of the wanted entry
	 * @param entries
	 *            Sorted List of AbstractEntry
	 * @param lowerCase
	 *            If true, entries must be sorted case insensitive
	...
	public static int getEntry(String entryname, List<? extends AbstractEntry> entries, boolean lowerCase) {
		...
		if (lowerCase) {
			// This case is a bit more complicated since there could be
			// different entries with the
			...
			while (m < entries.size() && entries.get(m).getkey(lowerCase).equals(lEntryname)) {
				if (entries.get(m).key.equals(entryname))
					return m;
				m++;
			}
			return -1;
	...
	/**
	 * Search the given (sorted) array of entries for all entries, for which the
	 * start of the key matches the given search string.
	 * 
	 * This version uses binary search for finding the lower and upper bounds,
	 * resulting in O(log n) performance.
	 * 
	 * @param start
	 *            The start of the searchable string
	 * @param entries
	...
	/**
	 * Search the given (sorted) list of entries for all entries, for which the
	 * start of the key matches the given search string.
	 * 
	 * This version uses binary search for finding the lower and upper bounds,
	 * resulting in O(log n) performance.
	 ...
	 *            The entries to search
	 * @param initBounds
	 *            The initial lower and upper bounds to start the search from
	 * @param lowerCase
	 *            If true, assumes that the list is sorted lower case and
	 * @return A two-element array with the lower (inclusive) and upper
	 *         (exclusive) bounds or {-1,-1} if no matching entries were found.
	 */
	protected int[] getCompletionsBin(String start, List<? extends AbstractEntry> entries, int[] initBounds,
			boolean lowerCase) {
