...
public class HardLineWrap {
	 ...
	 * String.
	 * 
	 * Examples: "   hello world" will return "hello world" "   % hello" will
	 * return "hello" "   %hello" will return "hello" "   % % hello" will return
	 * "% hello" "   %% hello" will return "% hello"
	 * 
	 * @param str
	 *            The string to trim
	 * @return trimmed version of the string
	 ...
	 * first white space after <code> MAX_LENGTH</code>. Normally it adds the
	 * rest of the currentline to the next line. Exceptions are empty lines,
	 * commandlines, commentlines, and special lines like \\ or \[.
	 * 
	 * @param d
	 *            IDocument
	 * @param c
	 *            DocumentCommand
	 * @param MAX_LENGTH
	 *            How many characters are allowed at one line.
	 */
	public void doWrapB(IDocument d, DocumentCommand c, int MAX_LENGTH) {
		try {
			...
			// String indent = tools.getIndentation(d, c); // TODO check if
			// inside comment
			...
			// Figure out whether the next line should be merged with the
			// wrapped text
			...
			if (line.trim().endsWith(".") || line.trim().endsWith(":") || line.trim().endsWith("\\\\")) {
				...
				if (tools.getIndexOfComment(line) >= 0 // wrapped text contains
														// a comment,
						&& tools.isLineCommentLine(nextTrimLine) // next line is
																	// also a
																	// comment
																	// line,
						&& tools.getIndentation(line).equals(tools.getIndentation(nextline)) // with
																								// the
																								// same
																								// indentation!
						&& !isSingleLine(trimBeginPlusComment(nextTrimLine))) // but
																				// not
																				// an
																				// empty
																				// comment
																				...
																				// etc.
				{
					...
					isWithNextline = true;
					// 3th case: Wrapped text is comment, next line is not
					// (otherwise case 2)
			...
			// TODO: if line has a comment at the end, this might be wrapped
			// onto a non-comment line
