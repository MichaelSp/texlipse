...
/**
 * For now we use OutlineNode.getName() for comparing the nodes and for
 * determining the hashCode for a node. Obviously this is not very smart and has
 * some unwanted side effects.
 * 
 * The better solution would be to use the positions instead of node names.
 * However for some unknown reason, the offset and length of the old positions
 * do not get updated and thus the position is not equal to the new position.
 * 
