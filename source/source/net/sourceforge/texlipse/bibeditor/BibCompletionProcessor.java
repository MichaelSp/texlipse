...
/**
 * Handles abbrev and template completions for the BibTeX editor.
 * 
 * @author Oskar Ojala
 */
public class BibCompletionProcessor implements IContentAssistProcessor {
	 ...
	 * 
	 * @param model
	 *            The document model this processor is associated with.
	 */
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		 ...
		 * ICompletionProposal[] repeats = null; String line = ""; // Determine
		 * the begining of the line for (int i = offset - 1; i > 0; i--) { char
		 * c = completeDoc.charAt(i); // FIXME what if we're inside a string? if
		 * (c == '\n' || (c == ',' && completeDoc.charAt(i - 1) =='}')) { line =
		 * completeDoc.substring(i + 1, offset); break; } }
		 * 
		 * // try to split the line into a field type and a field value String[]
		 * lineparts = line.split("="); if (lineparts.length == 2) { // FIXME
		 * this fails if the line is foo = "foo = bar", String field =
		 * lineparts[0].trim(); String value = lineparts[1].trim().substring(1);
		 * repeats = computeRepeatCompletions(offset, value.length(), value,
		 * field); }
		 */
		// ----------------------
		...
		if (completeStart == -1) {
			...
			abbrevs = computeAbbrevCompletions(offset, offset - completeStart,
					completeDoc.substring(completeStart, offset));
		}

		// make combined list of repeats, abbrev and template completion
		// proposals
		int size = 0;
		// if (repeats != null)
		// size += repeats.length;
		...
		if (size == 0) {
			...
			/*
			 * 
			 * if (repeats != null) { for (int i=0; i < repeats.length; i++) {
			 * value[index] = repeats[i]; index++; } }
			 */
			// ----------------------
			if (abbrevs != null) {
	 ...
	 * @param prefix
	 *            The start of the abbreviation or ""
	...
	private ICompletionProposal[] computeAbbrevCompletions(int offset, int replacementLength, String prefix) {
		...
		for (int i = 0; i < abbrevs.length; i++) {
			result[i] = new CompletionProposal(abbrevs[i].key, offset - replacementLength, replacementLength,
					abbrevs[i].key.length(), null, abbrevs[i].key, null, abbrevs[i].info);
		}
		return result;
	}
	 ...
	 * @return An array containing all possible completions
	 */
	// private ICompletionProposal[] computeRepeatCompletions(int offset, int
	// replacementLength, String prefix, String field) {
	//
	...
	//
	// // Special case the author field, as it is a list of elements separated
	// by "and"
	// if ((field.equalsIgnoreCase("author") ||
	// field.equalsIgnoreCase("editor")) && prefix.contains(" and ")) {
	...
	/**
	 * Resolves the latest word immediately before the cursor position and
	 * returns it
	 * 
	 * @param doc
	 *            Document to examine
	 ...
	 * @return the offset index,from where the latest word begins (if 0, then
	 *         there is no latest word)
	 */
	 ...
	 *            The start of the abbreviation or ""
	 * @param viewer
	 *            The viewer associated with this document
	 * @return An array containing all possible completions
	 */
	private ICompletionProposal[] computeTemplateCompletions(int offset, int replacementLength, String prefix,
			ITextViewer viewer) {
		List<ICompletionProposal> templateProposals = new ArrayList<ICompletionProposal>();
		...
		for (Iterator<ICompletionProposal> iter = templateProposals.iterator(); iter.hasNext();) {
