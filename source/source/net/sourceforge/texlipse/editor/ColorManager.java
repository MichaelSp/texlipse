...
/**
 * Color cache for the Latex file editor. These colors will be used in syntax
 * coloring, the text background, for example, is defined in the editor
 * preferences.
 * 
...
public class ColorManager {
	...
	// ".TexColor" -suffix is there because these names are used also in the
	// preferences
	...
	// private static final RGB DEFAULT_TEX_WHITE_COLOR = new RGB(125, 125,
	// 125);
	...
	protected Map<String, Object> fColorTable = new HashMap<String, Object>(10);
	protected Map<?, ?> fStyleTable = new HashMap<Object, Object>(10);
	protected Color bgColor;

	/**
	 * Initialize default colors and styles to preferences. This should be
	 * called only from PreferenceInitializer.
	 * 
	 * @param preferences
	 *            preferences
	 */
	public static void initializeDefaults(IPreferenceStore preferences) {
		...
		// PreferenceConverter.setDefault(preferences, TEX_WHITE,
		// DEFAULT_TEX_WHITE_COLOR);
	...
	/**
	 * Returns the color object for some color. The color object may not
	 * represent the same color as the parameter.
	 * 
	 * @param rgb
	...
	public Color getColor(String rgb) {
		...
		if (color == null) {
			color = new Color(Display.getCurrent(), PreferenceConverter.getColor(TexlipsePlugin.getDefault()
					.getPreferenceStore(), rgb));
	 ...
	 */
	public void dispose() {
		Iterator<Object> e = fColorTable.values().iterator();
		while (e.hasNext())
