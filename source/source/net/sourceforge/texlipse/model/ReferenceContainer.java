...
/**
 * Container for referencing data (BibTeX and labels.) Holds the reference lists
 * of each file as well as a (case insensitive) sorted array of all references,
 * so that not all files need to be reparsed when the data changes.
 * 
 * @author Oskar Ojala
...
public class ReferenceContainer {
	...
	/**
	 * Adds a reference source <code>refs</code> associated with
	 * <code>key</code> into this object. Does not update the sorted array of
	 * references.
	 * 
	 * @param key
	 *            The key associated with these references in the internal set
	 * @param refs
	...
	/**
	 * Updates the contents of this object if the given key exists in the
	 * internal set. Updating includes replacing the old data with the new and
	 * re-creating the sorted reference array.
	 * 
	 * If the key doesn't exist, then nothing is done. Thus, this method is
	 * suitable for use by the BibTeX editor to update the BibTeX references in
	 * case these are used by the LaTeX editor.
	 * 
	 * @param key
	 *            The key with which to associate the reference source
	 * @param refs
	 ...
	 * whether the new keyset has changed or not. If it has, the names of the
	 * new keys are returned as a list. If the internal set contains keys which
	 * aren't in the given keyset, then the these kesy and their datastructures
	 * are removed from the internal set.
	 * 
	 * The naming of this method is due to the fact that it's really useful for
	 * BibTeX files (ie. keys), but not for labels, as the BibTeX files are all
	 * defined in one command and thus fetched simultaneously, whereas labels
	 * are built incrementally.
	 * 
	 * @param newBibs
	 *            An array containing the new keys to store into the set
	 * @return An empty list if there were no new keys, otherwise a list
	 *         containing the names of the new keys
	 */
	...
	/**
	 * Checks whether this container is fresh: it is up to date if it contains
	 * exactly the same keys (or in practice BibTeX-files) as those given as
	 * arguments.
	 * 
	 * @param newBibs
	 *            Array of the new keys
	 * @return <code>true</code> if this container is up to date, false
	 *         otherwise
	 */
	...
	/**
	 * Takes a list of errors (<code>DocumentReference</code>) and checks which
	 * of them are "false alarms" (ie. they exist in this container)
	 * 
	 ...
	 * organizing this container, this method should be used to find out whit
	 * references are actually resolved.
	 * 
	 * @param errors
	 *            A list of errors in the form of <code>DocumentReference</code>
	 */
