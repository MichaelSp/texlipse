...
public class TexOutlinePage extends ContentOutlinePage {
	...
	/**
	 * Updates the outline with new content. Called by TexDocumentModel through
	 * the editor. Saves the visible state of the outline, sets the new content
	 * and restores the state.
	 * 
	 * @param input
	 *            the new outline input
	 */
		...
		if (viewer != null) {
			...
			if (control != null && !control.isDisposed()) {
				...
				if (!succUpdate) {
					...
					/*
					 * viewer.getContentProvider().inputChanged(viewer, null,
					 * input.getRootNodes()); viewer.refresh(true);
					 */
	...
	/**
	 * Gets the text of the currently selected item. Use by copy paste and
	 * drag'n'drop operations.
	 * 
	 * @return text of the currently selected item or null if no item is
	 *         selected
	 * 
	...
	public String getSelectedText() {
		...
		String text;
		try {
			text = this.editor.getDocumentProvider().getDocument(this.editor.getEditorInput())
					.get(pos.getOffset(), pos.getLength());
	...
	/**
	 * Removes the text of the currently selected item From the document. Used
	 * by copy paste and drag'n'drop operations.
	 * 
	...
	public void removeSelectedText() {
		...
		try {
			this.editor.getDocumentProvider().getDocument(this.editor.getEditorInput())
					.replace(pos.getOffset(), pos.getLength(), "");
		} catch (BadLocationException e) {
			return;
		}
	...
	/**
	 * Called by the TexDocumentModel when it gets dirty. Enables the update
	 * button.
	 */
	...
	/**
	 * Initialize copy paste by getting the clipboard and hooking the actions to
	 * global edit menu.
	 * 
	 * @param viewer
	 */
	 ...
	 * 
	 * @param the
	 *            IMenuManager of the context menu
	 */
	...
	/**
	 * Removes own SelectionChangeListener from TreeViewer and uses listener
	 * instead Needed for Full LaTeX Outline
	 */
	 ...
	 * 
	 * @param nodeType
	 *            the type of nodes to be revealed
	 */
	...
	/*
	 * private void restoreExpandState(OutlineNode newNode, ArrayList oldNodes,
	 * ArrayList newNodes) {
	 * 
	 * // check this node OutlineNode oldNode; for(Iterator iter =
	 * oldNodes.iterator(); iter.hasNext();) { oldNode =
	 * (OutlineNode)iter.next(); if (newNode.likelySame(oldNode)) {
	 * //System.out.println(newNode.getName() + " LIKE " + oldNode.getName());
	 * newNodes.add(newNode); iter.remove(); } }
	 * 
	 * // continue with children ArrayList children = newNode.getChildren(); if
	 * (children != null) { for (Iterator iter = children.iterator();
	 * iter.hasNext();){ restoreExpandState((OutlineNode)iter.next(), oldNodes,
	 * newNodes); } } }
	 */
}