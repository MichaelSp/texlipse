...
/**
 * Viewer configuration editor. This dialog can be used for editing old configs
...
public class ViewerConfigDialog extends Dialog {
	...
	private static final String[] inverseSearchValues = new String[] { ViewerAttributeRegistry.INVERSE_SEARCH_NO,
			ViewerAttributeRegistry.INVERSE_SEARCH_RUN, ViewerAttributeRegistry.INVERSE_SEARCH_STD };
	 ...
	 * 
	 * @param parentShell
	 *            shell of the creating component
	 */
	 ...
	 *            shell of the creating component
	 * @param nameList
	 *            list of existing viewer configuration names
	 */
	 ...
	 * 
	 * @param key
	 *            message text key on resource bundle
	 * @param info
	 *            value of the text parameter (%s)
	 */
	...
	/**
	 * Check that the config is valid. Close the dialog is the config is valid.
	 */
						...
						for (int i = 0; i < configs.length; i++) {
							...
							if (c.getType().getIdentifier().equals(TexLaunchConfigurationDelegate.CONFIGURATION_ID)) {
								if (c.getAttribute("viewerCurrent", "").equals(name)) {
									// We've found a config which was based on
									// this viewer
									if (0 == returnCode) {
										String message = MessageFormat.format(TexlipsePlugin
												.getResourceString("preferenceViewerUpdateConfigurationQuestion"),
												new Object[] { c.getName() });
										md = MessageDialogWithToggle
												.openYesNoCancelQuestion(
														getShell(),
														TexlipsePlugin
																.getResourceString("preferenceViewerUpdateConfigurationTitle"),
														message,
														TexlipsePlugin
																.getResourceString("preferenceViewerUpdateConfigurationAlwaysApply"),
														false, null, null);
									...
									// If answer was yes, update each config
									// with latest values from registry
									if (returnCode == IDialogConstants.YES_ID) {
										ILaunchConfigurationWorkingCopy workingCopy = c.getWorkingCopy();
										workingCopy.setAttributes(registry.asMap());

										// We need to set at least one attribute
										// using a one-shot setter method
										// because the method setAttributes does
										// not mark the config as dirty.
										// A dirty config is required for a
										// doSave to do anything useful.
										workingCopy.setAttribute("viewerCurrent", name);
										workingCopy.doSave();
									}

									// Reset return-code if we should be asked
									// again
	...
	private void addInverseChooser(Composite parent) {
		...
		String[] list = new String[] { TexlipsePlugin.getResourceString("preferenceViewerInverseSearchNo"),
