...
public class TexCommandContainer {
	...
	public static final TexCommandEntry[] builtIn = {
			...
			new TexCommandEntry("addcontentsline",
					"\\addcontentsline{}{}{}\n\nAdds a chapter title to the contents page.", 3),
			new TexCommandEntry(
					"alph",
					"\\alph{counter}\n\nCauses the current value of counter to be printed in alphabetic lowercase characters, i.e., a, b, c... The \\Alph command causes upper case alphabetic characters, i.e., A, B, C...",
					1),
			new TexCommandEntry(
					"Alph",
					"\\Alph{counter}\n\nCauses the current value of counter to be printed in alphabetic uppercase characters, i.e., A, B, C... The \\alph command causes upper case alphabetic characters, i.e., a, b, c...",
					1),
			new TexCommandEntry(
					"appendix",
					"\\appendix\n\nChanges the way sectional units are numbered. The command generates no text and does not affect the numbering of parts.",
					0),
			new TexCommandEntry(
					"arabic",
					"\\arabic{counter}\n\nCauses the current value of counter to be printed in arabic numbers, i.e., 3.",
					1),
			new TexCommandEntry("author",
					"\\author{name}\n\nDefines the author, used for output if \\maketitle is used.", 1),
			new TexCommandEntry("backslash",
					"\\backslash\n\nOutputs a backslash in math mode. (Enclose by two dollar symbols.)", 0),
			new TexCommandEntry("begin", "\\begin{envname}\n\nStart of an environment, must end with \\end{envname}.",
					1),
			new TexCommandEntry("bfseries", "\\bfseries\n\nSwitch to a bold font.", 0),
			new TexCommandEntry(
					"bibitem",
					"\\bibitem{cite key}{item}\n\n\\bibitem requires two arguments for parsing. LaTeX syntax allows writing this as if it were two arguments, even though it is only one. This command is used within a \\thebibliography environment.",
					2),
			new TexCommandEntry(
					"bibliography",
					"\\bibliography{filename}\n\nIncludes the bibliography at this point. The argument should be the name of the BibTeX file to use without the .bib -extension. Multiple files can be used by separating them with commas.",
					1),
			new TexCommandEntry(
					"bibliographystyle",
					"\\bibliographystyle{}\n\nDefines the style file to use for the bibliography generation with BibTeX",
					1),
			new TexCommandEntry(
					"bigskip",
					"\\bigskip\n\nEquivalent to \\vspace{\\bigskipamount} where \\bigskipamount is determined by the document style.",
					0),
			new TexCommandEntry(
					"caption",
					"\\caption{}\n\nUsed within a \\figure or \\table environment to specify a caption. May be followed by a \\label command.",
					1),
			new TexCommandEntry("cdots", "\\cdots\n\nOutputs 3 dots.", 0),
			new TexCommandEntry("center", "\\center{text}\n\nCenters a block of text.", 1),
			new TexCommandEntry(
					"centering",
					"\\centering\n\nThis declaration can be used inside an environment such as quote or in a parbox. The text of a figure or table can be centered on the page by putting a \\centering command at the beginning of the figure or table environment.",
					0),
			new TexCommandEntry(
					"centerline",
					"\\centerline{text}\n\nCenters a text line.\nThis is a TeX Command, better use {\\centering text}.",
					1),
			new TexCommandEntry("chapter",
					"\\chapter{}\n\nOutputs a chapter heading. \\chapter* does not add a contents entry.", 1),
			new TexCommandEntry("cite",
					"\\cite{reference key}\n\nCites a reference. The reference key must be defined in a .bib file.", 1),
			new TexCommandEntry("comment",
			...
			new TexCommandEntry(
					"description",
					"\\description{}\n\nA list environment. Each \\item of the list should be followed by square-bracketed text, which will be highlighted.",
					1),
			new TexCommandEntry(
					"documentclass",
					"\\documentclass{class}\n\nSpecifies the main style of the document (report, article, ?). Use \\chapter in reports and \\section in articles.",
					1),
			new TexCommandEntry("em", "\\em\n\nSwitch to a emphasize (italic) font.", 0),
			new TexCommandEntry("emph", "\\emph{text}\n\nEmphasizes (italic) text.", 1),
			new TexCommandEntry("end",
			...
			new TexCommandEntry(
					...
					1),
			new TexCommandEntry("enumerate",
					"\\enumerate{}\n\nSets numbers to the \\items used in the list environment in question.", 1),
			new TexCommandEntry(
					"figure",
					"\\figure{}\n\nAn environment for figures. Allows interpretation of embedded caption commands as figures.",
					1),
			...
			new TexCommandEntry(
					"fnsymbol",
					"\\fnsymbol{counter}\n\nCauses the current value of counter to be printed in a specific sequence of nine symbols that can be used for numbering footnotes. The sequence is asterisk, dagger, double dagger, section mark, paragraph mark, double vertical lines, double asterisks, double daggers, double double daggers.",
					1),
			new TexCommandEntry("footnote", "\\footnote{}\n\nCreates a footnote to the end of the section.", 1),
			new TexCommandEntry(
					"footnotemark",
					"\\footnotemark[number]\n\nPuts the footnote number in the text. It is used in conjunction with \\footnotetext to produce footnotes where \\footnote cannot be used.",
					0),
			new TexCommandEntry(
					"footnotetext",
					"\\footnotetext[number]{text}\n\nUsed in conjunction with the \\footnotemark command and places the text in the argument at the bottom of the page. This command can come anywhere after the \\footnotemark command.",
					1),
			new TexCommandEntry("hline",
			...
			new TexCommandEntry("index",
					"\\index{keyword}\n\nAdds a keyword to the keyword list of the current topic.", 1),
			new TexCommandEntry("input", "\\input{filename}\n\nIncludes the specified file.", 1),
			new TexCommandEntry("insertatlevel",
					"\\insertatlevel{level}{text}\n\nInserts the text into the given level of the document.", 2),
			new TexCommandEntry(
					"item",
					"\\item\n\nMarks an item of a list. Each \\item of the list may be followed by square-bracketed text, which will be highlighted.",
					0),
			new TexCommandEntry("itemize", "\\itemize{}\n\nIndents each \\item of the list in question with a bullet.",
					1),
			...
			new TexCommandEntry(
					"label",
					"\\label{label}\n\nLabels the chapter, section, subsection or figure caption with the given label.",
					1),
			...
			new TexCommandEntry(
					"maketitle",
					"\\maketitle\n\nCreates the article or report title by combining the \\title, \\author and optionally \\date.",
					0),
			...
			new TexCommandEntry("multicolumn",
					"\\multicolumn{}{}{}\n\nUsed in \\tabular environment to denote a multicolumn cell.", 3),
			new TexCommandEntry(
					"newcommand",
					"\\newcommand\n\nDefines a new command. An optional argument between the two mandatory ones defines the number of arguments of the command. Arguments can be referred in the definition as #1, #2 etc.",
					2),
			new TexCommandEntry(
					"newenvironment",
			...
			new TexCommandEntry(
					"pagenumbering",
					"\\pagenumbering{argument}\n\nalph is [a,b,?], Alph is [A,B,?], arabic is [1,2,?], roman is [i,ii,?], Roman is [I,II,?]",
					1),
			new TexCommandEntry("pageref", "\\pageref{label}\n\nCreates a page reference to the given label.", 1),
			new TexCommandEntry(
					"pagestyle",
					"\\pagestyle{option}\n\nChanges the style from the current page on throughout the remainder of your document. Valid options are plain, empty, headings and myheadings.",
					1),
			new TexCommandEntry("par", "\\par\n\nEnds paragraph.", 0),
			new TexCommandEntry("part", "\\part{title}\n\nStarts a new part of a book.", 1),
			new TexCommandEntry(
					"paragraph",
					"\\paragraph\n\nStarts a new paragraph with the specified heading.\nEssentially a lower level sectioning command than subsubsection.",
					0),
			new TexCommandEntry(
					"parindent",
					"\\parindent{indentation}\n\nIndents the first line of the next paragraphs by the specified amount.",
					1),
			...
			new TexCommandEntry(
					"providecommand",
					"\\providecommand\n\nDefines a new command if the command has not been defined previously. An optional argument between the two mandatory ones defines the number of arguments of the command. Arguments can be referred in the definition as #1, #2 etc.",
					2),
			new TexCommandEntry(
					"put",
					"\\put(x-coord,y-coord){object}\n\nPlaces the object specified by the mandatory argument at the given coordinates. The coordinates are in units of \\unitlength. Some of the object items you might place are \\line, \\vector, \\circle and \\oval.",
					1),
			...
			new TexCommandEntry("ref",
					"\\ref{}\n\nRefers to a \\label and causes the number of that section or figure to be printed.", 1),
			new TexCommandEntry(
					"renewcommand",
					"\\renewcommand\n\nRedefines a command. An optional argument between the two mandatory ones defines the number of arguments of the command. Arguments can be referred in the definition as #1, #2 etc.",
					2),
			new TexCommandEntry(
					"renewenvironment",
			...
			new TexCommandEntry(
					"roman",
					"\\roman{counter}\n\nCauses the current value of counter to be printed in lower case roman numerals, i.e., i, ii, iii, ... The \\Roman command causes upper case roman numerals, i.e., I, II, III...",
					1),
			new TexCommandEntry(
					"Roman",
					"\\Roman{counter}\n\nCauses the current value of counter to be printed in upper case roman numerals, i.e., I, II, III, ... The \\roman command causes upper case roman numerals, i.e., i, ii, iii...",
					1),
			...
			new TexCommandEntry(
					"shortcite",
					"\\shortcite{reference key}\n\nCites a reference. The reference key must be defined in a .bib file.",
					1),
			...
			new TexCommandEntry("tableofcontents",
					"\\tableofcontents\n\nInserts a table of contents to the current location.", 0),
			new TexCommandEntry(
					"tabular",
					"\\tabular{}{}\n\nTabular environment: The first argument specifies the column formatting. a pipe symbol (|) denotes a vertical border, one of l, r, c signifies a normal column of default width, and p followed by a dimension specifies a column of given width.",
					2),
			...
			new TexCommandEntry("textnormal", "\\textnormal{text}\n\nOutputs the text in the standard document font.",
					1),
			...
			new TexCommandEntry(
					"thebibliography",
					"\\thebibliography{}\n\nAn environment for specifying the bibliography as a series of \\bibitem commands.",
					1),
			new TexCommandEntry(
					"thispagestyle",
					"\\thispagestyle{option}\n\nWorks in the same manner as the \\pagestyle command except that it changes the style for the current page only.",
					1),
			...
			new TexCommandEntry(
					"verb",
					"\\verb{argument}\n\nFormats the given small amount of text to a fixed-width font without interpreting any LaTeX commands.",
					1),
			new TexCommandEntry(
					"verbatim",
					"\\verbatim{argument}\n\nFormats the given argument to a fixed-width font without interpreting any LaTeX commands.",
					1),
			new TexCommandEntry(
					"verbatiminput",
					"\\verbatiminput{filename}\n\nInclude the given file as if it were within a \\verbatim environment.",
					1),
			new TexCommandEntry(
					"vspace",
	...
	public static final TexCommandEntry[] greekSmall = {
			...
			new TexCommandEntry("varepsilon", "\\varepsilon\n\nPrints a small greek epsilon",
					"mathSym/greek/varepsilon"),
	...
	public static final TexCommandEntry[] miscMath = {
			new TexCommandEntry("label", "\\label{label}\n\nLabels the current equation line with the given label.", 1,
					TexCommandEntry.MATH_CONTEXT),
			new TexCommandEntry("nonumber",
					"\\nonumber\n\nThe preceding formula in an eqnarray environment gets no number.", 0,
					TexCommandEntry.MATH_CONTEXT),
	...
	public static final TexCommandEntry[] stdArrows = {
			new TexCommandEntry("leftarrow", "\\leftarrow\n\nSimple arrow which points to the left",
					"mathSym/arrows/leftarrow"),
			new TexCommandEntry("gets", "\\gets\n\nSimple arrow which points to the left", "mathSym/arrows/leftarrow"),
			new TexCommandEntry("Leftarrow", "\\Leftarrow\n\nDouble arrow which points to the left",
					"mathSym/arrows/bigleftarrow"),
			new TexCommandEntry("rightarrow", "\\rightarrow\n\nSimple arrow which points to the right",
					"mathSym/arrows/to"),
			new TexCommandEntry("to", "\\to\n\nSimple arrow which points to the right", "mathSym/arrows/to"),
			new TexCommandEntry("Rightarrow", "\\Rightarrow\n\nDouble arrow which points to the right",
					"mathSym/arrows/bigto"),
			new TexCommandEntry("leftrightarrow",
					"\\leftrightarrow\n\nSimple arrow which points to the left and the right",
					"mathSym/arrows/leftrightarrow"),
			new TexCommandEntry("Leftrightarrow",
					"\\Leftrightarrow\n\nDouble arrow which points to the left and the right",
					"mathSym/arrows/bigleftrightarrow"),
			new TexCommandEntry("mapsto", "\\mapsto\n\nSimple mapping arrow which points to the right",
					"mathSym/arrows/mapsto"),

			new TexCommandEntry("hookleftarrow", "\\hookleftarrow\n\nHooked arrow which points to the left",
					"mathSym/arrows/hookleftarrow"),
			new TexCommandEntry("hookrightarrow", "\\hookrightarrow\n\nHooked arrow which points to the right",
					"mathSym/arrows/hookrightarrow"),
			new TexCommandEntry("leftharpoonup", "\\leftharpoonup\n\nHarpon arrow which points to the left",
					"mathSym/arrows/leftharpoonup"),
			new TexCommandEntry("leftharpoondown", "\\leftharpoondown\n\nHarpon arrow which points to the left",
					"mathSym/arrows/leftharpoondown"),
			new TexCommandEntry("rightharpoonup", "\\rightharpoonup\n\nHarpon arrow which points to the right",
					"mathSym/arrows/rightharpoonup"),
			new TexCommandEntry("rightharpoondown", "\\rightharpoondown\n\nHarpon arrow which points to the right",
					"mathSym/arrows/rightharpoondown"),
			new TexCommandEntry("leftrightharpoons",
					"\\leftrightharpoons\n\nHarpon arrow which points to the left and the right",
					"mathSym/arrows/leftrightharpoons"),

			new TexCommandEntry("longleftarrow", "\\longleftarrow\n\nLong arrow which points to the left",
					"mathSym/arrows/longleftarrow"),
			new TexCommandEntry("Longleftarrow", "\\Longleftarrow\n\nLong double arrow which points to the left",
					"mathSym/arrows/biglongleftarrow"),
			new TexCommandEntry("longrightarrow", "\\longrightarrow\n\nLong arrow which points to the right",
					"mathSym/arrows/longrightarrow"),
			new TexCommandEntry("Longrightarrow", "\\Longrightarrow\n\nLong double arrow which points to the right",
					"mathSym/arrows/biglongrightarrow"),
			new TexCommandEntry("longleftrightarrow",
					"\\longleftrightarrow\n\nLong arrow which points to the left and the right",
					"mathSym/arrows/longleftrightarrow"),
			new TexCommandEntry("Longleftrightarrow",
					"\\Longleftrightarrow\n\nLong double arrow which points to the left and the right",
					"mathSym/arrows/biglongleftrightarrow"),
			new TexCommandEntry("longmapsto", "\\longmapsto\n\nLong mapping arrow which points to the right",
					"mathSym/arrows/longmapsto"),
			...
			new TexCommandEntry("downarrow", "\\downarrow\n\nSimple arrow which points down",
					"mathSym/arrows/downarrow"),
			new TexCommandEntry("Downarrow", "\\Downarrow\n\nDouble arrow which points down",
					"mathSym/arrows/bigdownarrow"),
			new TexCommandEntry("updownarrow", "\\updownarrow\n\nSimple arrow which points up and down",
					"mathSym/arrows/updownarrow"),
			new TexCommandEntry("Updownarrow", "\\Updownarrow\n\nDouble arrow which points up and down",
					"mathSym/arrows/bigupdownarrow"),

			new TexCommandEntry("nearrow", "\\nearrow\n\nSimple arrow which points north east",
					"mathSym/arrows/nearrow"),
			new TexCommandEntry("searrow", "\\searrow\n\nSimple arrow which points south east",
					"mathSym/arrows/searrow"),
			new TexCommandEntry("swarrow", "\\swarrow\n\nSimple arrow which points south west",
					"mathSym/arrows/swarrow"),
			new TexCommandEntry("nwarrow", "\\nwarrow\n\nSimple arrow which points north west",
					"mathSym/arrows/nwarrow"),
			new TexCommandEntry("leadsto", "\\leadsto\n\ns-shaped arrow which points to the right",
					"mathSym/arrows/leadsto") };
	...
	/**
	 * Adds a new reference source (tex file) into this container or updates an
	 * existing one.
	 * 
	 * @param key
	 *            The name of the reference source (filename)
	 * @param refs
	 *            The commands to insert
	...
	/**
	 * Searches (very inefficient) for the last entries of each context and
	 * saves them in contexts
	 */
	private void createContexts() {
		...
		for (int i = 0; i < sortedCommands.size(); i++) {
			if (current != ((TexCommandEntry) sortedCommands.get(i)).context) {
				...
				current++;
				if (current == TexCommandEntry.NUMBER_OF_CONTEXTS)
					break;
			}
		}
	...
	/**
	 * Organizes this container's contents into the sorted command array. The
	 * added reference sources' commands are added to the built-in commands and
	 * sorted.
	 */
	public void organize() {
		...
		// copy all commands and change the context to activate them also in
		// mathmode
		...
		for (TexCommandEntry c : builtIn)
			sortedCommands.add(c);
		sortedCommands.addAll(allRefs);
		for (TexCommandEntry c : greekCapital)
			sortedCommands.add(c);
		for (TexCommandEntry c : greekSmall)
			sortedCommands.add(c);
		for (TexCommandEntry c : miscMath)
			sortedCommands.add(c);
		for (TexCommandEntry c : stdArrows)
			sortedCommands.add(c);
		for (TexCommandEntry c : stdCompare)
			sortedCommands.add(c);
		for (TexCommandEntry c : functionNames)
			sortedCommands.add(c);
		for (TexCommandEntry c : stdBinOpSymbols)
			sortedCommands.add(c);
		for (TexCommandEntry c : stdBraces)
			sortedCommands.add(c);
		for (TexCommandEntry c : stdAccents)
			sortedCommands.add(c);
