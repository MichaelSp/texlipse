...
/**
 * A LaTeX word counting parser. Counts the likely printed words from the given
 * string, ie. all normal words and the contents of sectioning commands are
 * counted. Cite-references are counted as one word.
 * 
 * @author Oskar Ojala
 */
public class LatexWordCounter {
	 ...
	 * 
	 * @param selection
	 *            The string to use for counting words
	 */
	...
	/**
	 * Counts the number of (LaTeX) words in the string that this object
	 * contains.
	 * 
			...
			for (Token t = lexer.next(); !(t instanceof EOF); t = lexer.next()) {
				if (expectArg) {
					if (t instanceof TArgument) {
						...
						expectArg = false;
					} else if (!(t instanceof TOptargument) && !(t instanceof TWhitespace) && !(t instanceof TStar)
					...
					if (t instanceof TWord || t instanceof TCcite) {
						if (!"&".equals(t.getText()))
							words++;
					} else if (t instanceof TWhitespace) { // make the common
															// case fast
						continue;
					} else if (t instanceof TCpart || t instanceof TCchapter || t instanceof TCsection
							|| t instanceof TCssection || t instanceof TCsssection || t instanceof TCparagraph)
						expectArg = true;
				}
			}
			return words;
