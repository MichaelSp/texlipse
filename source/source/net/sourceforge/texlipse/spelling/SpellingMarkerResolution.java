...
public class SpellingMarkerResolution implements IMarkerResolution2 {
	 ...
	 * 
	 * @param begin
	 *            beginning offset in the document
	 * @param len
	 *            length of the replacable area
	 * @param str
	 *            replacement string (might be of different length than the
	 *            replacable area)
	 * @param doc
	 *            the document where the replacing occurs
	 */
	...
	public void run(IMarker marker) {
		...
		if (charBegin > 0 && str.length() < charEnd
				&& (!Character.isWhitespace(str.charAt(charBegin - 1)) || !Character.isWhitespace(str.charAt(charEnd)))) {
			charBegin++;
			charEnd++;
