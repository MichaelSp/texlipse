...
/**
 * A server that takes in filename-linenumber commands and passes these commands
 * as events to its listener.
 * 
 * The server will listen to the specified socket until it gets a command
 * "QUIT".
 * 
 * @author Esa Seuranen
...
public class FileLocationServer implements Runnable {
	...
	// the listener, which will be notified every time a file location command
	// occurs
	 ...
	 * 
	 * @param listener
	 *            the new file location listener.
	 */
	...
	public void run() {
		...
		ssocket = null;
		try {
			ssocket = new ServerSocket(TexlipsePlugin.getDefault().getPreferenceStore()
					.getInt(TexlipseProperties.FILE_LOCATION_PORT));
		} catch (IOException e) {
	...
	/**
	 * Try to parse a "filename:linenumber" -combination and notify listener if
	 * parsing is successful.
	 * 
	 * @param line
	 *            the line of characters to parse
	 */
