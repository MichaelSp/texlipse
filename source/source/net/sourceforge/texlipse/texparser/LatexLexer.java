...
/**
 * Lexer for LaTeX -files. Implements tokenizing curly brace-enclosed areas and
...
public class LatexLexer extends Lexer {
	 ...
	 * 
	 * @param in
	 *            The reader to read the character stream from
	 */
	...
	protected void filter() throws LexerException {
		...
		if (state.equals(State.COMCAPT)) {
			...
			if (token instanceof TCnew) {
				commandDef = true;
			} else if (token instanceof TCword && !commandDef && !defCommands.contains(token.getText().substring(1))) {
				state = State.NORMAL;
				return;
			}
			...
			if (argStart == null) {
				...
				if (token instanceof TLBrace)
					...
					throw new LexerException("[" + argStart.getLine() + "," + (argStart.getPos() - 1)
							+ "] There's a } missing: unexpected end of file");
				}
				if (count != 0) {
					...
					TArgument targ = new TArgument(text.toString(), argStart.getLine(), argStart.getPos());
					// emit the string
					token = targ;
					state = State.COMCAPT;
					argStart = null;
			...
			if (argStart == null) {
				...
				if (token instanceof TLBrace)
					...
					throw new LexerException("[" + argStart.getLine() + "," + argStart.getPos()
							+ "] There's a } or a ] missing: unexpected end of file");
				}
				...
				if (count != 0 || !(token instanceof TRBracket)) {
					...
					TOptargument tsl = new TOptargument(text.toString(), argStart.getLine(), argStart.getPos());
					// emit the string
					token = tsl;
					state = State.COMCAPT;
					argStart = null;
			...
			if (token instanceof TVtext) {
				...
				throw new LexerException("[" + argStart.getLine() + "," + argStart.getPos()
						+ "] The verb-command isn't closed: unexpected end of file");
			}
