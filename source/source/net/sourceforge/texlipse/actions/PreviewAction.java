...
/**
 * Preview action runs the previewer (that is configured in preferences) on the
 * current project.
 * 
...
public class PreviewAction implements IWorkbenchWindowActionDelegate, IEditorActionDelegate {
	...
	/**
	 * Launches either the most recent viewer configuration, or if there is no
	 * previous viewers, creates a new viewer launch configuration.
	 */
	public void run(IAction action) {
		try {
			...
			String outputFormat = TexlipseProperties.getProjectProperty(project, TexlipseProperties.OUTPUT_FORMAT);
			...
			if (config == null) {
				...
				// For some reason the Eclipse API wants us to init default
				// values
	...
	/**
	 * Cache the window object in order to be able to provide UI components for
	 * the action.
	 */
