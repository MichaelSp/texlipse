...
public class TexStyleCompletionManager implements IPropertyChangeListener {
	...
	private String[] STYLETAGS = new String[] { "\\textbf{", "\\textit{", "\\textrm{", "\\textsf{", "\\textsc",
			"\\textsl{", "\\texttt{", "\\emph{", "{\\huge", "{\\Huge" };
	private String[] STYLELABELS = new String[] { "bold", "italic", "roman", "sans serif", "small caps", "slanted",
			"teletype", "emphasize", "huge", "Huge" };
	...
	private void readSettings() {
		...
		for (int i = 0; i < props.length; i++) {
			...
			// continue;
			// }
			// keyValue.put(keys[i].substring(0, index),
			// keys[i].substring(index+1));
	...
	public ICompletionProposal[] getStyleCompletions(String selectedText, Point selectedRange) {
		 ...
		 * ICompletionProposal[keyValue.size()]; int i=0; for (Iterator iter =
		 * keyValue.keySet().iterator(); iter.hasNext();) { String key =
		 * (String) iter.next(); String value = (String) keyValue.get(key);
		 * 
		 * 
		 * String replacement = "{" + key + " " + selectedText + "}"; int cursor
		 * = key.length() + 2; IContextInformation contextInfo = new
		 * ContextInformation(null, value+" Style"); result[i] = new
		 * CompletionProposal(replacement, selectedRange.x, selectedRange.y,
		 * cursor, null, value, contextInfo, replacement); i++; }
		 */
		...
		for (int i = 0; i < STYLETAGS.length; i++) {
			...
			// Construct proposal
			result[i] = new CompletionProposal(replacement, selectedRange.x, selectedRange.y, cursor, null,
