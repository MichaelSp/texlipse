...
/**
 * Reconcile strategy used for spell checking TeX files. Most parts copied from
 * {@link org.eclipse.ui.texteditor.spelling.SpellingReconcileStrategy}. We
 * could not extend it, because of a bug in reconcile.
 */
public class TeXSpellingReconcileStrategy implements IReconcilingStrategy, IReconcilingStrategyExtension {

	/**
	 * Spelling problem collector. Copied from SpellingReconcileStrategy and
	 * changed it a little bit to ignore errors in TeX Commands.
	 */
	public static class TeXSpellingProblemCollector implements ISpellingProblemCollector {
		...
		/*
		 * @see org.eclipse.ui.texteditor.spelling.ISpellingProblemCollector#
		 * beginCollecting()
		 */
	...
	public void reconcile(DirtyRegion dirtyRegion, IRegion subRegion) {
		try {
			...
			if (startLineInfo.getOffset() == endLineInfo.getOffset())
				...
				subRegion = new Region(startLineInfo.getOffset(), endLineInfo.getOffset() + endLineInfo.getLength()
						- startLineInfo.getOffset());
			// Check everything from startLine to the end of the document,
			// otherwise
			// The positions of the errors are not in sync
			// subRegion= new Region(startLineInfo.getOffset(),
			// fDocument.getLength() - startLineInfo.getOffset());
