 ...
 * information from them. The front-end provides an upper-level interface to the
 * parser and does some pre-processing and error-handling.
 * 
 * @author Oskar Ojala
 */
public class TexParser {
	...
	/**
	 * @param input
	 *            The string representing the document to parse
	 */
	...
	/**
	 * Removes trailing whitespace from the document. This is needed since the
	 * lexer and Eclipse have a different view of how the positions in the
	 * trailing whitespace work out, so it's best to just remove it.
	 * 
	 * @param input
	...
	/**
	 * Extracts the preamble (if there is any) and stores a copy of it in the
	 * field <code>preamble</code>. The preamble is assumed to exist if the
	 * string contain the \documentclass command and it ends where the
	 * document-environment begins or at the end of file.
	 * 
	...
	private void extractPreamble(String input) {
		/*
		 * // These regexps lead to stack overflows in the regexp parser in some
		 * occasions.
		 * 
		 * //
		 * (?:\r|\n|^)(?:(?:\\%|[^%\r\n])*?(?:\\%|[^\\%]))?\\document(?:class|
		 * style)(?:\W|$) Pattern docclass = Pattern.compile(
		 * "(?:\\r|\\n|^)(?:(?:\\\\%|[^%\\r\\n])*?(?:\\\\%|[^\\\\%]))?\\\\document(?:class|style)(?:\\W|$)"
		 * ); Matcher m = docclass.matcher(input); if (m.find()) {
		 * 
		 * //
		 * (?:\r|\n|^)(?:(?:\\%|[^%\r\n])*?(?:\\%|[^\\%]))?\\begin\s*\{document
		 * \} Pattern begindoc = Pattern.compile(
		 * "(?:\\r|\\n|^)(?:(?:\\\\%|[^%\\r\\n])*?(?:\\\\%|[^\\\\%]))?\\\\begin\\s*\\{document\\}"
		 * ); Matcher m2 = begindoc.matcher(input); if (m2.find(m.end() - 1)) {
		 * this.preamble = input.substring(0, m2.end()); return; } }
		 * this.preamble = null; return;
		 */
	...
	public void parseDocument(String input, boolean checkForMissingSections) throws IOException {
		...
		try {
			...
			if (this.preamble != null) {
				OutlineNode on = new OutlineNode("Preamble", OutlineNode.TYPE_PREAMBLE, 1, null);
			...
			this.errors.add(new ParseErrorMessage(Integer.parseInt(numbers[0]), Integer.parseInt(numbers[1]), 2, msg
	...
	/**
	 * @return True if there were fatal errors due to which parsing couldn't be
