...
public class BuilderSettingsPreferencePage extends PreferencePage implements IWorkbenchPreferencePage {
	...
	private void addTexInstallDir(Composite contents) {
		...
		TexlipsePreferencePage.addLabelField(3, TexlipsePlugin.getResourceString("preferenceBuilderTexDirDescription"),
				parent);
	...
	private void addConsoleCheckBox(Composite contents) {
		...
		// WorkbenchHelp.setHelp(consoleOutputCheckBox,
		// TexlipseHelpIds.BUILDER_CONSOLE);
	}
	...
	/**
	 * Called when ok-button (or apply-button) is pressed. Saves all the field
	 * editor values to preferences.
	 * 
	 * @return true
	 */
	...
	/**
	 * Called when defaults-button is pressed. Sets all field values to
	 * defaults. Currently does nothing to builder paths.
	 */
	public void performDefaults() {
		super.performDefaults();
		consoleOutputCheckBox.setSelection(getPreferenceStore().getDefaultBoolean(
				TexlipseProperties.BUILDER_CONSOLE_OUTPUT));
		auxParserCheckBox.setSelection(getPreferenceStore().getDefaultBoolean(
				TexlipseProperties.BUILDER_PARSE_AUX_FILES));
	}
	 ...
	 * 
	 * @param index
	 *            index of the configurable builder
	 */
