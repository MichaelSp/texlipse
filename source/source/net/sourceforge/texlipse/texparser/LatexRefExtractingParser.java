...
/**
 * A LaTeX parser for extracting labels, BibTeX -information and whether an
 * index is to be generated or not.
 * 
 * @author Oskar Ojala
 */
public class LatexRefExtractingParser {
	 ...
	 * 
	 * @param input
	 *            A string containing the LaTeX document
	 * @throws IOException
			...
			for (Token t = lexer.next(); !(t instanceof EOF); t = lexer.next()) {
				if (expectArg) {
					if (t instanceof TArgument) {
						...
						// this is an error condition, but we want a silent
						// parse
						prevToken = null;
						expectArg = false;
					}
				} else if (expectArg2) {
					// we are capturing the second argument of a command with
					// two arguments
					...
					if (t instanceof TClabel || t instanceof TCbib || t instanceof TCbibstyle || t instanceof TCnew
							|| t instanceof TCpackage) {
						prevToken = t;
						expectArg = true;
