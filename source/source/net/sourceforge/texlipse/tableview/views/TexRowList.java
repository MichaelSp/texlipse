...
public class TexRowList {
	/**
	 * Constant for defining up direction for function sum(int column, int row,
	 * int direction)
	 */
	public final static int SUM_UP = 1;
	/**
	 * Constant for defining down direction for function sum(int column, int
	 * row, int direction)
	 */
	public final static int SUM_DOWN = 2;
	/**
	 * Constant for defining left direction for function sum(int column, int
	 * row, int direction)
	 */
	public final static int SUM_LEFT = 3;
	/**
	 * Constant for defining right direction for function sum(int column, int
	 * row, int direction)
	 */
	...
	private Vector<TexRow> rows = new Vector<TexRow>(ROW_COUNT);
	...
	// change listeners
	private Set<ITexRowListViewer> changeListeners = new HashSet<ITexRowListViewer>();

	// Vector for holding TexRows
	public Vector<TexRow> getRows() {
		return rows;
	}
	...
	public TexRow addRow() {
		...
		Iterator<ITexRowListViewer> iterator = changeListeners.iterator();
	 ...
	 * 
	 * @param index
	 *            where the row is inserted
	 * @param row
	 *            the TexRow which is inserted
	...
	/**
	 * Removes the given row (first occurrance) from the row list (or does
	 * nothing, if the row is not in the row list)
	 * 
	 * @param row
	 *            to be removed
	 */
	public void removeRow(TexRow row) {
		rows.remove(row); // FIXME now maxrowcount becomes one less, since the
							// row isn't cleared
		Iterator<ITexRowListViewer> iterator = changeListeners.iterator();
		while (iterator.hasNext())
	 ...
	 * 
	 * @param row
	 *            which was changed (and should be updated in table viewer)
	 */
	...
	/*
	 * Reads an table item from a string representing a LaTeX table row. The
	 * items separated by '&' characters (exlcluding the combnation "\&").
	 * 
	 * @param source LateX table row
	 * 
	 * @param index (of the source string) from which the search for the next
	 * item is started
	 * 
	 ...
	 * The rows are separeted by string "\\", and the last line does not need to
	 * have "\\"
	 * 
	...
	/**
	 * Interprets the given selection as simple LaTeX table and imports in to
	 * the given location. If the row is empty, then the import is done to that
	 * row, otherwise a new row is created and inserted and the import is done
	 * to that row.
	 * 
	 * Comments (i.e. "LaTeXTableStuff %ThisPartOfTheLineIsRemoved") are
	 * ignored, as well as "\.*line.*" parts.
	 * 
	 * @param selection
	 *            TexSelection object
	 * @param importAt
	 *            the starting row (index) for the import
	 * 
	 */
	public void importSelection(TexSelections selection, int importAt) {
		...
		while (index < s.length()) {
			...
			// We need this to ensure that the last empty field will be
			// recognized (Case: &\\)
			if (line.endsWith("&"))
				line = line + " ";
			index2 = 0;
	...
	/**
	 * Exports the table editor's content to clipboard in LaTeX table format.
	 * 
	...
	/**
	 * Returns the table editor's content in a raw, tabulated format suitable
	 * for e.g. gnuplot.
	 * 
	 * @return string representing the whole table
	 */
	...
	/**
	 * Flips the rows and columns of the table, i.e. 1 2 3 4 5 6 becomes 1 4 2 5
	 * 3 6
	 */
	public void flipRowsAndColumns() {
		...
		if (rows.size() >= TexRow.COLUMNS) {
			TexlipsePlugin.stat("Flipping rows and columns in LaTeX Table View for more than "
					+ Integer.toString(TexRow.COLUMNS) + " rows is not supported.");
			return;
		}
		for (int column = 0; column < TexRow.COLUMNS; column++) {
			for (int row = 0; ((row < column) && (row < rows.size())); row++) {
				...
				if (texRow.getCol(column).length() > 0)
					swap = true;
				if ((rows.size() > column) && ((TexRow) rows.get(column)).getCol(row).length() > 0)
					swap = true;
				...
				if (swap) {
					...
					texRow.setCol(column, ((TexRow) rows.get(column)).getCol(row));
	...
	/**
	 * Mirrors the columns of the table, i.e. 1 2 3 4 5 6 becomes 3 2 1 6 5 4
	 */
	...
	/**
	 * Mirrors the rows of the table, i.e. 1 2 3 4 5 6 becomes 4 5 6 1 2 3
	 */
	...
	/**
	 * Sums the cells in given area, defined with top left cell and the areas
	 * width and height
	 * 
	 * @param column
	 *            top left cell's column
	 * @param row
	 *            top left cell's row
	 * @param width
	 *            the are width (the number of cells)
	 * @param height
	 *            the are height (the number of cells)
	 */
	...
	/**
	 * Sums the cells in given direction from the defined cell (by column and
	 * row). The cell itself is not included into the sum
	 * 
	 * @param column
	 *            cell's column
	 * @param row
	 *            top left cell's row
	 * @param direction
	 *            one of SUM_UP, SUM_DOWN, SUM_LEFT, SUM_RIGHT
	 */
