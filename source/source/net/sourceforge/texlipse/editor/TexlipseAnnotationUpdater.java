...
public class TexlipseAnnotationUpdater implements ISelectionChangedListener {
	 ...
	 * via <code>addPostSelectionChangedListener</code>
	 * 
	 * @param editor
	 *            The TexEditor
	 */
	public TexlipseAnnotationUpdater(AbstractTextEditor editor) {
		// Add this listener to the current editors IPostSelectionListener (lazy
		// update)
		((IPostSelectionProvider) editor.getSelectionProvider()).addPostSelectionChangedListener(this);
		fEditor = editor;
		fEnabled = TexlipsePlugin.getDefault().getPreferenceStore()
				.getBoolean(TexlipseProperties.TEX_EDITOR_ANNOTATATIONS);
	 ...
	 * or \end{...} string. If the last is true, it searches with regular
	 * expressions to find the associated part(s) and highlights them (The last
	 * uses a non UI-Job which do not influence the responsiveness of the
	 * editor).
	 * 
	 * @param viewer
	 */
	private void update(ISourceViewer viewer) {
		...
		if (testSelection(selection, model))
			return;
		...
		if (selection instanceof ITextSelection) {
			try {
				// TODO Split this and create new classes for the different
				// annotations
				...
				if ("\\begin".equals(command) || "\\end".equals(command)) {
					...
					final IRegion startRegion = new Region(lineOff + r.getOffset(), r2.getOffset() + r2.getLength()
							- r.getOffset() + 1);
	...
	/**
	 * Creates and returns a background job which searches and highlights all
	 * \label and \*ref.
	 * 
	 * @param document
	 * @param model
	...
	/**
	 * Creates and returns a new background job which searches and highlights
	 * the matching \end or \begin environment.
	 * 
	 * @param document
	 * @param model
	 * @param offset
	 *            The offset of the selection (cursor)
	 * @param command
	 *            \begin or \end
	 * @param startRegion
	 *            A region which contains the command and the argument (e.g
	 *            \begin{environment})
	 * @param envName
		...
		return new Job("Update Annotations") {
			public IStatus run(IProgressMonitor monitor) {
				...
				if ("\\begin".equals(command))
					forward = true;
				if (forward) {
					IRegion endRegion = LatexParserUtils.findMatchingEndEnvironment(text, envName,
							startRegion.getOffset());
					if (endRegion != null) {
						createNewAnnotation(endRegion, "Environment", model);
						createNewAnnotation(startRegion, "Environment", model);
					}
				} else {
					IRegion endRegion = LatexParserUtils.findMatchingBeginEnvironment(text, envName,
							startRegion.getOffset());
					if (endRegion != null) {
						createNewAnnotation(endRegion, "Environment", model);
						createNewAnnotation(startRegion, "Environment", model);
	...
	private boolean testSelection(ISelection selection, IAnnotationModel model) {
		if (selection instanceof ITextSelection) {
			...
			for (Iterator<Annotation> iter = fOldAnnotations.iterator(); iter.hasNext();) {
				...
				if (p != null && p.offset <= textSelection.getOffset()
						&& p.offset + p.length >= textSelection.getOffset()) {
					return true;
				}
			}
		}
	 ...
	 * 
	 * @param r
	 *            The IRegion which should be highlighted
	 * @param annString
	 *            The name of the annotation (not important)
	 * @param model
	 *            The AnnotationModel
	 */
