...
public class BibOutlineContainer {
	...
	private List<BibOutlineContainer> childContainers;
	 ...
	 *            The initial entries
	 * @param topLevel
	 *            Whether or not this represents the top level of the hierarchy
	 */
	...
	private BibOutlineContainer(List<ReferenceEntry> initialEntries, String sName, String eName) {
		this.childEntries = initialEntries;
		this.startName = sName;
		this.endName = eName;
		this.topLevel = false;
	}

	public BibOutlineContainer(ArrayList<BibOutlineContainer> children, String sName, String eName) {
	...
	/**
	 * Makes a copy of this container so that first level children are copied as
	 * new objects but lower levels are not
	 * 
	 * @param sorting
	 *            The type of sorting that the new container will have
	...
	private BibOutlineContainer childCopy(String sorting) {
		...
		for (Iterator<ReferenceEntry> iter = childEntries.iterator(); iter.hasNext();) {
	...
	public BibOutlineContainer buildAuthorSort() {
		...
		for (Iterator<ReferenceEntry> iter = newboc.childEntries.iterator(); iter.hasNext();) {
			ReferenceEntry re = iter.next();
	...
	// private static final Pattern rmBraces =
	// Pattern.compile("(^|[^\\\\])(:?\\{|\\})");
	...
	private void authorSort() {
		...
		for (Iterator<ReferenceEntry> iter = childEntries.iterator(); iter.hasNext();) {
			...
			for (int i = 0; i < authors.length; i++) {
				if (authors[i].indexOf(',') == -1 && !(authors[i].startsWith("{") && authors[i].endsWith("}"))) {
	...
	public BibOutlineContainer buildIndexSort() {
		...
		// Collections.sort(newboc.childEntries, new
		// Comparator<ReferenceEntry>() {
		// public int compare(ReferenceEntry A, ReferenceEntry B) {
	...
	/**
	 * Calculate the shortest differentiating prefix between the strings that is
	...
	public void partition() {
		...
		for (int i = 0; i < totalPartitions; i++) {
			int partitionSize = Math.min(childEntries.size() - MAX_PARTITIONSIZE * i, MAX_PARTITIONSIZE);
			...
			// BibOutlineContainer boc = new BibOutlineContainer(children, pre1
			// + "..." + pre2);
		...
		while (bottomContainers.size() > MAX_PARTITIONSIZE) {
			...
			while (bottomContainers.size() > 0) {
				...
				// if it doesn't work...
				ArrayList<BibOutlineContainer> children = new ArrayList<BibOutlineContainer>();
				ListIterator<BibOutlineContainer> liter = bottomContainers.listIterator();
				for (int j = 0; j < partitionEnd; j++) {
					children.add(liter.next());
					liter.remove();
				}

				String sName = children.get(0).startName;
				String eName = children.get(0).endName;
	 ...
	 */
	public List<BibOutlineContainer> getChildContainers() {
