...
public class SpellingCompletionProposal implements ICompletionProposal {
	...
	public void apply(IDocument document) {
		try {
			...
			if (this.solution.equals(SpellChecker.SPELL_CHECKER_ADD)) {
				...
				// replace word in document only if user chose a replacement
				// word
	...
	/**
	 * Returns the new selection after the proposal has been applied to the
	 * given document in absolute document coordinates. If it returns
	 * <code>null</code>, no new selection is set.
	 * 
	 * A document change can trigger other document changes, which have to be
	 * taken into account when calculating the new selection. Typically, this
	 * would be done by installing a document listener or by using a document
	 * position during {@link #apply(IDocument)}.
	 * 
	 * @param document
	 *            the document into which the proposed completion has been
	 *            inserted
	...
	/**
	 * Returns optional additional information about the proposal. The
	 * additional information will be presented to assist the user in deciding
	 * if the selected proposal is the desired choice.
	 * 
	...
	/**
	 * Returns optional context information associated with this proposal. The
	 * context information will automatically be shown if the proposal has been
	 * applied.
	 * 
