...
/**
 * Configuration for the source viewer of the LaTeX editor.
 * 
 * @author Oskar Ojala
...
public class TexSourceViewerConfiguration extends TextSourceViewerConfiguration {
	...
	public IContentAssistant getContentAssistant(ISourceViewer sourceViewer) {
		...
		// TexCompletionProcessor tcp = new
		// TexCompletionProcessor(this.editor.getDocumentModel());
		...
		// assistant.setContentAssistProcessor(tcp,
	...
	/**
	 * Defines a default partition scanner and sets the default color for it
	 * 
	...
	/**
	 * Defines a math partition scanner and sets the default color for it.
	 * 
	...
	protected TexMathScanner getTeXMathScanner() {
		if (mathScanner == null) {
			...
			mathScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager.getColor(ColorManager.EQUATION),
	...
	protected TexCommentScanner getTexCommentScanner() {
		if (commentScanner == null) {
			...
			commentScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager
					.getColor(ColorManager.COMMENT), null, colorManager.getStyle(ColorManager.COMMENT_STYLE))));
		}
		return commentScanner;
	}

	/**
	 * Defines an argument (curly bracket) scanner and sets the default color
	 * for it
	 * 
	...
	protected TexArgScanner getTexArgScanner() {
		if (argumentScanner == null) {
			...
			argumentScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager
					.getColor(ColorManager.CURLY_BRACKETS), null, colorManager
					.getStyle(ColorManager.CURLY_BRACKETS_STYLE))));
		}
		return argumentScanner;
	}

	/**
	 * Defines an optional argument (square bracket) scanner and sets the
	 * default color for it
	 * 
	 * @return a scanner to detect argument partitions
	 */
	protected TexOptArgScanner getTexOptArgScanner() {
		if (optArgumentScanner == null) {
			...
			optArgumentScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager
					.getColor(ColorManager.SQUARE_BRACKETS), null, colorManager
					.getStyle(ColorManager.SQUARE_BRACKETS_STYLE))));
		}
		return optArgumentScanner;
	}

	/**
	 * Defines a tikz environment or single line scanner and sets the default
	 * color for it
	 * 
	 * @return a scanner to detect argument partitions
	 */
	protected TexTikzScanner getTexTikzScanner() {
		if (tikzScanner == null) {
			...
			tikzScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager.getColor(ColorManager.DEFAULT),
					null, colorManager.getStyle(ColorManager.DEFAULT_STYLE))));
		}
		return tikzScanner;
	}
	...
	protected RuleBasedScanner getTexVerbatimScanner() {
		if (verbatimScanner == null) {
			// We need no rules, because the user can write everything inside a
			// verbatim env
			verbatimScanner = new RuleBasedScanner();
			verbatimScanner.setDefaultReturnToken(new Token(new TextAttribute(colorManager
					.getColor(ColorManager.VERBATIM), null, colorManager.getStyle(ColorManager.VERBATIM_STYLE))));
		}
	...
	private static final DefaultInformationControl.IInformationPresenter presenter = new DefaultInformationControl.IInformationPresenter() {
		public String updatePresentation(Display display, String infoText, TextPresentation presentation, int maxWidth,
				int maxHeight) {
			int cstart = -1;
			int gstart = -1;
