 ...
 * 
 *         This class implements user made text selections. Selection has
 *         different features line indexes, selection string, startline etc.
 *         This class is used for example commenting blocks feature.
 */
public class TexSelections {
	...
	/**
	 * Takes a text editor as a parameter and sets variables to correspond
	 * selection features.
	 * 
	 * @param textEditor
	 *            The currenct text editor
	 */
	...
	private void select() {
		...
		try {
			// If anything is actually selected, we'll be modifying the
			// selection only
			if (this.selLength > 0) {
	...
	public void selectParagraph() {
		if (textSelection.getLength() == 0) {
			try {
				...
				Pattern p = Pattern.compile("(?m)" + Pattern.quote(endLineDelim) + "\\p{Blank}*"
	...
	public String getCompleteLines() {
		try {
			return document.get(startLine.getOffset(),
					endLine.getOffset() + this.endLine.getLength() - this.startLine.getOffset());
		} catch (Exception e) {
	...
	/**
	 * Returns complete selection as a text String, ie. if selectCompleteLines
	 * have been used
	 * 
	...
	/**
	 * Returns the length of the current selection, which is extended to the
