...
public class TexCorrectIndentationAction implements IEditorActionDelegate {
	...
	private void indent() throws BadLocationException {
		...
		try {
			lines = document.get(document.getLineOffset(selection.getStartLineIndex()), selection.getSelLength())
					.split(delimiter);
		...
		for (int line = selection.getStartLineIndex(); index < lines.length; line++) {
			...
			if (line == 0 || line == document.getLength() - 1) {
				...
				if (Arrays.binarySearch(this.indentationItems, this.tools.getEnvironment(endText)) >= 0) {
					int matchingBegin = this.tools.findMatchingBeginEquation(document, line,
							this.tools.getEnvironment(endText));

					if (matchingBegin < selection.getStartLineIndex()) {
						correctIndentation = this.tools.getIndentation(document, matchingBegin, "\\begin",
								this.tabWidth);
					} else {
						correctIndentation = this.tools.getIndentation(
								lines[matchingBegin - selection.getStartLineIndex()], this.tabWidth);
					}
				}
