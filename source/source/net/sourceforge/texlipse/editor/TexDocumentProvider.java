...
/**
 * Simple document provider implementation that adds latex nature to the project
 * holding a newly created file. This way we also get latex builder and
 * properties to any project that contains latex files.
 * 
...
public class TexDocumentProvider extends TextFileDocumentProvider {
	...
	public TexDocumentProvider() {
		...
		provider = new ForwardingDocumentProvider(TexEditor.TEX_PARTITIONING, new TexDocumentSetupParticipant(),
				provider);
	...
	public IDocument getDocument(Object element) {
		...
		// this way we also get latex builder to any project that has latex
		// files
			...
			if (file != null) {
				...
				try {
					if (!project.hasNature(TexlipseNature.NATURE_ID)) {
						...
						// this will cause the builder to be added, if not
						// already there
						n.configure();
					}
				...
				if (format == null || format.length() == 0) {
					...
					TexlipseProperties.setProjectProperty(project, TexlipseProperties.BUILDER_NUMBER,
					...
					String output = name.substring(0, name.lastIndexOf('.') + 1)
							+ TexlipsePlugin.getPreference(TexlipseProperties.OUTPUT_FORMAT);
