...
public class BibCodeFolder {
	...
	private ArrayList<BibProjectionAnnotation> oldNodes;
	 ...
	 */
	public void update(List<ReferenceEntry> outline) {
	 ...
	 *            The outline data structure containing the document positions
	 */
	private void addMarks(List<ReferenceEntry> outline) {
		if (firstRun) {
			Map<BibProjectionAnnotation, Position> map = new HashMap<BibProjectionAnnotation, Position>();
			...
			oldNodes = new ArrayList<BibProjectionAnnotation>();

			// save old nodes
			for (@SuppressWarnings("unchecked")
			Iterator<BibProjectionAnnotation> iter = model.getAnnotationIterator(); iter.hasNext();) {
	 ...
	 *            The outline data structure containing the document positions
	 */
	private void markTreeNodes(List<ReferenceEntry> outline) {
		markloop: for (ListIterator<ReferenceEntry> iter = outline.listIterator(); iter.hasNext();) {
			ReferenceEntry re = iter.next();

			Position pos = re.position;
			for (ListIterator<BibProjectionAnnotation> li2 = oldNodes.listIterator(); li2.hasNext();) {
				BibProjectionAnnotation cAnnotation = li2.next();
	...
	/**
	 * Fills the given annotation map with all the annotations from this
	 * document.
	 * 
	 * @param documentTree
	 *            Document tree representing this document
	 * @param map
	 *            The annotation map to fill
	 * @param fold
	 *            Whether entries should be set as folded or not
	 */
	private void fillAnnotationMap(List<ReferenceEntry> documentTree, Map<BibProjectionAnnotation, Position> map,
			boolean fold) {
		for (ListIterator<ReferenceEntry> iter = documentTree.listIterator(); iter.hasNext();) {
			ReferenceEntry node = iter.next();
