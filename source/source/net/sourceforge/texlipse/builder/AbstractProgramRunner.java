...
public abstract class AbstractProgramRunner implements ProgramRunner {
	...
	private final ExternalProgram extrun;
	...
	protected String getArguments(IResource resource) {
		...
		if (args.indexOf("%fullinput") >= 0) {
			args = args.replaceAll("%fullinput", resource.getParent().getLocation().toFile().getAbsolutePath()
					+ File.separator + inputName);
		}
		if (args.indexOf("%fulloutput") >= 0) {
			args = args.replaceAll("%fulloutput", resource.getParent().getLocation().toFile().getAbsolutePath()
	 ...
	 */
	protected static void createMarker(IResource resource, Integer lineNumber, String message, String markerType,
			int severity) {
		int lineNr = -1;
		if (lineNumber != null) {
			lineNr = lineNumber;
