 ...
 * 
 * The list of strings is converted into a single String object by putting a
 * separator character between the items while concatenating.
 * 
...
public class StyleListFieldEditor extends FieldEditor implements IInputValidator {
	...
	private Map<String, String> smartKeys;
				...
				tableViewer.setSorter(new ViewerSorter() {
					public int compare(Viewer iviewer, Object e1, Object e2) {
						if (e1 == null) {
							...
							return ((SmartKeyVariable) e1).getName().compareToIgnoreCase(
									((SmartKeyVariable) e2).getName());
						}
					}
				});
			}
	...
	class KeyValueInputDialog extends MessageDialog {
		 ...
		 * @param defaultIndex
		 */
		public KeyValueInputDialog(Shell parentShell, String dialogTitle, Image dialogTitleImage, String dialogMessage,
		...
		/**
		 * Constructor with default values for many fields. This is the
		...
		public KeyValueInputDialog(Shell shell, String title, String message, IInputValidator vali) {
			this(shell, title, null, message, QUESTION, new String[] { IDialogConstants.OK_LABEL,
		...
		/**
		 * Creates and returns the contents of an area of the dialog which
		 * appears below the message and above the button bar.
		 * 
	...
	class SmartKeyVariableInputDialog extends MessageDialog {
		...
		/**
		 * Constructor with default values for many fields. This is the
		 * preferred way to construct this class.
		 * 
		 * @param shell
		 *            parent shell
		 * @param title
		...
		protected Point getInitialSize() {
			...
			p.x = (p.x - 100) * sum / maxLen + 100; // scale the list size down
													// by the average length of
													// an item
			p.y += fd[0].getHeight() * 8 * 2; // add 8 lines to the default
												// height (*2 to get it right.
	...
	public StyleListFieldEditor(String name, String labelText, Composite parent) {
		...
		smartKeys = new HashMap<String, String>();
	}
	 ...
	 * 
	 * @param newText
	 *            the contents of the dialog's text field
	...
	protected Composite createTable(Composite parent) {
		...
		// Create table
		smartKeyTable = new TableViewer(tableComposite, SWT.BORDER | SWT.H_SCROLL | SWT.V_SCROLL | SWT.MULTI
				| SWT.FULL_SELECTION);
