...
public class TexAutoIndentStrategy extends DefaultIndentLineAutoEditStrategy {
			...
			public void propertyChange(PropertyChangeEvent event) {
				...
				if (TexlipseProperties.WORDWRAP_LENGTH.equals(ev)) {
					...
					autoItem = fPreferenceStore.getBoolean(TexlipseProperties.TEX_ITEM_COMPLETION);
						...
						|| TexlipseProperties.INDENTATION_TABS.equals(ev) || TexlipseProperties.INDENTATION.equals(ev)
	...
	public void customizeDocumentCommand(IDocument document, DocumentCommand command) {
		...
		if (TexAutoIndentStrategy.hardWrap && command.length == 0 && command.text != null) {
			try {
				final String contentType = ((IDocumentExtension3) document).getContentType(TexEditor.TEX_PARTITIONING,
						command.offset, true);
				...
				// If we cannot retrieve the current content type, do not break
				// lines
			}
		}
	}
	 ...
	 * @param coffset
	 *            The starting offset (just at the beginning of the
	 *            "\begin{...}"
	...
	private void smartIndentAfterNewLine(IDocument document, DocumentCommand command) {
		try {
			...
			// this is save
			String lineDelimiter = document.getLegalLineDelimiters()[TextUtilities.endsWith(
					document.getLegalLineDelimiters(), command.text)];
			int beginIndex;
			if ((beginIndex = LatexParserUtils.findCommand(startLine, "\\begin", 0)) != -1) {
				// test if line contains \begin and search the environment
				// (itemize,
				// table...)
				...
				/*
				 * String prevIndentation = this.tools.getIndentation(document,
				 * line, "\\begin", this.tabWidth); // NEW
				 */
				...
				/*
				 * looks for the \begin-statement and inserts an equivalent
				 * \end-statement (respects \begin-indentation)
				 */
	...
	/**
	 * Removes indentation if \end{...} is detected. We assume that command.text
	 * is the closing brace '}'
	 * 
	 * @param document
	 *            Document where new line is detected.
	 * @param command
	 *            Command that represent the change of the document (here
	 *            command text is "}").
	 */
