...
public class ExternalProgram {
	...
	public void stop() {
		if (process != null) {
			process.destroy();
			// can't null the process here, because run()-method of this class
			// is still executing
			// process = null;
		}
	}
	...
	/**
	 * Runs the external program as a process and waits for the process to
	 * finish execution.
	 * 
	 * @param queryMessage
	 *            text which will trigger the query dialog
	 ...
	 * 
	 * @param wait
	 *            if true, this method will block until the process has finished
	 *            execution
	 * @return the text produced to standard output by the process
	 * @throws IOException
	 */
	protected String run(boolean wait, String[] queryMessage) throws IOException {
		...
		if ((command != null) && (dir != null)) {
			...
			// the "path" environment variable, and xelatex needs its directory
			// in the path.
		...
		final OutputScanner scanner = new OutputScanner(process.getInputStream(), process.getOutputStream(),
				queryMessage, consoleOutput);

		// scan also the standard error stream
		final OutputScanner errorScanner = new OutputScanner(process.getErrorStream(), process.getOutputStream(),
				queryMessage, consoleOutput);
