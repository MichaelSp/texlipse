...
public class TexMathCompletionProcessor implements IContentAssistProcessor {
	...
	public ICompletionProposal[] computeCompletionProposals(ITextViewer viewer, int offset) {
		...
		int templatepos = Math.max(Math.max(lineStart.lastIndexOf(' '), lineStart.lastIndexOf('$')),
				lineStart.lastIndexOf('}')) + 1;

		String replacement = lineStart.substring(templatepos);

		ICompletionProposal[] templateProposals = computeTemplateCompletions(offset, replacement.length(), replacement,
				viewer);
		...
		if (backpos >= 0) {
			...
			if (!(command.indexOf(' ') >= 0 || command.indexOf('{') >= 0 || command.indexOf('(') >= 0)) {
				...
				if ("\\".equals(command) || "end".startsWith(command)) {
					...
					if (endString != null) {
						cp = new CompletionProposal("\\" + endString, offset - replacement.length(),
				...
				if (comEntries != null) {
					...
					for (int i = 0; i < comEntries.size(); i++) {
						proposals[start + i] = new TexCompletionProposal(comEntries.get(i), offset - len, len, fviewer);
					}
				} else {
					if (cp != null) {
