...
public class FastLaTeXPartitionScanner implements IPartitionTokenScanner {
	...
	public static final String[] TEX_PARTITION_TYPES = new String[] { IDocument.DEFAULT_CONTENT_TYPE, TEX_COMMENT,
			TEX_MATH, TEX_CURLY_BRACKETS, TEX_SQUARE_BRACKETS, TEX_VERBATIM, TEX_TIKZPIC };
	...
	private static final String[] MATHRULES = { "equation", "eqnarray", "align", "alignat", "flalign", "multline",
			"gather" };
	private static final String[] MATHRULESSTAR = { "equation*", "eqnarray*", "align*", "alignat*", "flalign*",
			"multline*", "gather*" };
	...
	private final IToken[] fTokens = new IToken[] { new Token(null), new Token(TEX_COMMENT), new Token(TEX_MATH),
			new Token(TEX_VERBATIM), new Token(TEX_CURLY_BRACKETS), new Token(TEX_SQUARE_BRACKETS),
	...
	public IToken nextToken() {
		...
		default:
			...
			while (ch != '$' && ch != '\\' && ch != '%' && ch != '{' && ch != '[' && ch != ICharacterScanner.EOF) {
				ch = fScanner.read();
				offsetEnd++;
			}
			if (ch != ICharacterScanner.EOF)
	...
	private static final int getEnvIndex(final String envName) {
		if (isMathEnv(envName))
			return MATH;
		else if (isVerbatimEnv(envName))
			return VERBATIM;
		else if (isCommentEnv(envName))
			return COMMENT;
		else if (isTikzEnv(envName))
			return TIKZPIC;
		else
			return TEX;
	}
	...
	public static final boolean isMathEnv(String envName) {
		return matchesAny(envName, MATHRULES) || matchesAny(envName, MATHRULESSTAR)
