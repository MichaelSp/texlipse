...
public class ViewerAttributeRegistry implements Cloneable {
	...
	// active viewer. This variable is not in the registry, because it is needed
	// all the time.
	...
	/**
	 * Construct a new copy of the viewer attributes, based on the default
	 * viewer defined in the preference pages (first on the list).
	 */
	...
	/**
	 * Finds a file from all of the directories listed in the "path" environment
	 * variable. Unix default is assumed to be "/usr/bin"
	 * 
	 * @param filename
	 *            filename in unix-based systems
	 * @param winFilename
	 *            filename in windows systems
	 * @param winPath
	 *            default path in windows systems
	 * @return
	 */
	...
	/**
	 * Set default values to the preferences. Intended to be called only once
	 * from the IPreferenceInitializer.
	 * 
	 * @param prefs
	 *            the preferences
	 */
	public static void initializeDefaults(IPreferenceStore prefs) {
		...
		prefs.setDefault(VIEWER_SUMATRA + ATTRIBUTE_COMMAND,
				findFromEnvPath("SumatraPDF", "SumatraPDF.exe", "C:\\Program Files\\SumatraPDF"));
		...
		prefs.setDefault(VIEWER_ACROBAT + ATTRIBUTE_DDE_VIEW_COMMAND,
	...
	public void setDefaults() {
		...
		registry.put(activeViewer + ATTRIBUTE_ARGUMENTS,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_ARGUMENTS));

		registry.put(activeViewer + ATTRIBUTE_DDE_VIEW_COMMAND,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_VIEW_COMMAND));
		registry.put(activeViewer + ATTRIBUTE_DDE_VIEW_SERVER,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_VIEW_SERVER));
		registry.put(activeViewer + ATTRIBUTE_DDE_VIEW_TOPIC,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_VIEW_TOPIC));
		registry.put(activeViewer + ATTRIBUTE_DDE_CLOSE_COMMAND,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_CLOSE_COMMAND));
		registry.put(activeViewer + ATTRIBUTE_DDE_CLOSE_SERVER,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_CLOSE_SERVER));
		registry.put(activeViewer + ATTRIBUTE_DDE_CLOSE_TOPIC,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_DDE_CLOSE_TOPIC));

		registry.put(activeViewer + ATTRIBUTE_FORMAT, TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_FORMAT));
		registry.put(activeViewer + ATTRIBUTE_INVERSE_SEARCH,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_INVERSE_SEARCH));
		registry.put(activeViewer + ATTRIBUTE_FORWARD_SEARCH,
				TexlipsePlugin.getPreference(activeViewer + ATTRIBUTE_FORWARD_SEARCH));
	}
	...
	public void load(IPreferenceStore pref) {
		...
		if (list != null && list.indexOf(',') > 0) {
			...
			for (int i = 0; i < names.length; i++) {
				...
				registry.put(names[i] + ATTRIBUTE_DDE_VIEW_COMMAND,
						pref.getString(names[i] + ATTRIBUTE_DDE_VIEW_COMMAND));
				...
				registry.put(names[i] + ATTRIBUTE_DDE_CLOSE_COMMAND,
						pref.getString(names[i] + ATTRIBUTE_DDE_CLOSE_COMMAND));
				registry.put(names[i] + ATTRIBUTE_DDE_CLOSE_SERVER,
						pref.getString(names[i] + ATTRIBUTE_DDE_CLOSE_SERVER));
	 ...
	 *            plugin preferences
	 * @param viewers
	 *            the currently configured viewers in the correct order
	 */
	 ...
	 * 
	 * @param format
	 *            file format (the file extension)
	 */
	 ...
	 * 
	 * @param regMap
	 *            hashmap containing new values for viewer attributes
	 */
	public void setValues(Map<String, String> regMap) {
		registry.putAll(regMap);
		activeViewer = regMap.get(VIEWER_CURRENT);
	}
	...
	public String[] getFormatList() {
		return new String[] { TexlipseProperties.OUTPUT_FORMAT_DVI, TexlipseProperties.OUTPUT_FORMAT_PS,
