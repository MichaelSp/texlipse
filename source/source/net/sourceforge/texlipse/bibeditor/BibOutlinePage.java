...
public class BibOutlinePage extends ContentOutlinePage {
	...
	private final HashMap<String, Action> outlineActions;
	 ...
	 * 
	 * @param textEditor
	 *            The editor that this outline is associated with
	 */
	public BibOutlinePage(ITextEditor textEditor) {
		super();
		this.outlineActions = new HashMap<String, Action>();
	...
	public void createControl(Composite parent) {
		...
		viewer.setContentProvider(new BibContentProvider(editor.getDocumentProvider().getDocument(
	 ...
	 * 
	 * @param content
	 *            The new content of the outline
	 */
	// public void update(Hashtable<String, BibStringTriMap<ReferenceEntry>>
	// sortIndex) {
	...
	private void createActions() {
		...
		// creates the additional actions from
		// "BibEditorOutlineExtension"-Extension point
		...
		if (config.length > 0) {
			...
			for (IConfigurationElement elem : config) {
				try {
					BibOutlineActionProvider a = (BibOutlineActionProvider) elem
							.createExecutableExtension("OutlineActionProvider");
		...
		Action byAuthor = new Action("Sort by Author", IAction.AS_RADIO_BUTTON) {
			...
			public void run() {
				if (isChecked()) {
					BibOutlineContainer newContent = ((BibContentProvider) getTreeViewer().getContentProvider())
							.changeSort(BibOutlineContainer.SORTAUTHOR);
					update(newContent);
				}
			}
		};
		...
		Action byJournal = new Action("Sort by Journal", IAction.AS_RADIO_BUTTON) {
			...
			public void run() {
				if (isChecked()) {
					BibOutlineContainer newContent = ((BibContentProvider) getTreeViewer().getContentProvider())
							.changeSort(BibOutlineContainer.SORTJOURNAL);
					update(newContent);
				}
			}
		};
		...
		Action byIndex = new Action("Sort by Index", IAction.AS_RADIO_BUTTON) {
			...
			public void run() {
				if (isChecked()) {
					BibOutlineContainer newContent = ((BibContentProvider) getTreeViewer().getContentProvider())
							.changeSort(BibOutlineContainer.SORTINDEX);
					update(newContent);
				}
			}
		};
		...
		Action byYear = new Action("Sort by Year", IAction.AS_RADIO_BUTTON) {
			...
			public void run() {
				if (isChecked()) {
					BibOutlineContainer newContent = ((BibContentProvider) getTreeViewer().getContentProvider())
							.changeSort(BibOutlineContainer.SORTYEAR);
					update(newContent);
				}
			}
		};
	...
	private void createToolbar() {
		...
		toolbarManager.add(outlineActions.get(ACTION_BYINDEX));
		toolbarManager.add(outlineActions.get(ACTION_BYAUTHOR));
		toolbarManager.add(outlineActions.get(ACTION_BYJOURNAL));
		toolbarManager.add(outlineActions.get(ACTION_BYYEAR));
		// adds the additional actions from the
		// "BibEditorOutlineExtension"-Extension point
		...
		if (ti != null && ti.length > 0) {
			...
			if (selectedElement instanceof ReferenceEntry) {
				if (((ReferenceEntry) selectedElement).refFile != null) {
					mgr.add(outlineActions.get(ACTION_OPENBIBREF));
				}
			}
		}
	}
