...
/**
 * Scans the input stream for the given trigger strings and produces a query
 * dialog if sees one.
 * 
...
public class OutputScanner {
	...
	/**
	 * Read output from the stream and save it into a buffer. If the trigger
	 * string occurs in the stream, a question dialog will be popped up.
	 * 
	 * Reading is done byte-by-byte because we don't know when the stream is
	 * going to pause. Reading by buffers would mean that we could end up
	 * waiting input from the program when the program is waiting input from us,
	 * i.e. a deadlock.
	 * 
	...
	public boolean scanOutput() {
		try {
			...
			while (true) {
				...
				// TriggerStrings can only occur if the program is waiting for
				// input => in.available() == 0
				...
				if (consoleOutput != null && (char) nextByte == '\n') {
					...
					int lf = 1;
					if (sb.charAt(sb.length() - 2) == '\r') { // fix for windows
																// linefeeds
						lf++;
					}
					// don't print the whole buffer as the printToConsole()
					// outputs a linefeed
