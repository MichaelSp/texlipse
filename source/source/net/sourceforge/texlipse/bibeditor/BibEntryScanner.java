...
/**
 * Syntax highlighting scanner for BibTeX-entries. This scanner is used to scan
 * the syntax inside entries.
 * 
 * @author Oskar Ojala
 */
public class BibEntryScanner extends RuleBasedScanner {
	...
	public class BibSeparatorRule implements IRule {
		 ...
		 * BibTeX-entry special character is encountered
		 * 
		 * @param token
	...
	public BibEntryScanner(BibColorProvider provider) {
		...
		IToken keyword = new Token(new TextAttribute(provider.getColor(BibColorProvider.KEYWORD)));
		IToken type = new Token(new TextAttribute(provider.getColor(BibColorProvider.TYPE)));
		IToken string = new Token(new TextAttribute(provider.getColor(BibColorProvider.STRING)));
