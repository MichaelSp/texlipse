...
/**
 * Extracts information from the .aux file which is created by a latex run. If
 * \include is used, several .aux-files exist. They are parsed recursively.
 * 
 ...
 * declared in self-defined latex commands, so the normal texlipse-parser does
 * not find them
 * 
	...
	private List<String> doParse(String filename, String command) {
		...
		try {
			for (Token t = lexer.next(); !(t instanceof EOF); t = lexer.next()) {
				...
				if (prevText != null) {
					if (prevText.getText().equalsIgnoreCase("input") && t instanceof TWord) {
						prevText = null;
