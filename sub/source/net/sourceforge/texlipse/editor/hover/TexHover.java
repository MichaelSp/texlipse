	...
	public IRegion getHoverRegion(ITextViewer textViewer, int offset) {
		try{
			...
			if (obr != -1 && cbr != -1 && obr > line.lastIndexOf('}', start) && 
				...
				if (obr != -1) return new Region(lOffset + obr, (cbr-obr)+1);
			}
			//else extract current word
			int dLength = line.length();
			while((start > 0) && !isIgnoreChar(line.charAt(start))) start--;
			while((finish < dLength) && !isIgnoreChar(line.charAt(finish))) finish++;
			//special case \command we want the backslash if not \\command
			if (line.charAt(start) == '\\' && (start == 0 || 
					(start > 0 && line.charAt(start-1) != '\\'))) start--;
