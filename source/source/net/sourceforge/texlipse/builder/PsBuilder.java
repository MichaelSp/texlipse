...
/**
 * Build a pdf file from tex file(s) using either pslatex+ps2pdf or
 * latex+dvips+ps2pdf.
 * 
...
public class PsBuilder extends AbstractBuilder {
	...
	public void stopRunners() {
		// stopRunners instead of stopBuild, because we didn't start a separate
		// thread for DviBuilder
		dvi.stopRunners();
		pdf.stop();
		stopped = true;
	}
	...
	public void buildResource(IResource resource) throws CoreException {
		// call buildResource directly, because we don't want a separate thread
		// for DviBuilder
		stopped = false;
