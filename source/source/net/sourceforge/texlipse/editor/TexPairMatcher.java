...
public class TexPairMatcher implements ICharacterPairMatcher {
	...
	// string of pairs, so that two consecutive characters form a pair and the
	// first
	 ...
	 * parameter so that two consecutive characters form a pair (in which the
	 * first character is "left" and the latter is "right.
	 * 
	...
	/**
	 * Clears all internal state information in preparation for
	 * <code>match</code>
	 * 
	...
	/**
	 * Matches smallest region between a pair. If there is another pair (of same
	 * type) inside the region, it is taken into consideration. Example (numbers
	 ...
	 * @param document
	 * @param offset
	 * @return region (the pair included) between the matching pair, or
	 *         <code>null</code> if there is no counterpair for the character at
	 *         the offset (either it is not a matching pair character or it's
	 *         pair does not exist)
	 * 
	 ...
	 * current position) is RIGHT (0) if the character is "right" character in a
	 * pair, or LEFT (1) if the current character is anything else.
	 * 
