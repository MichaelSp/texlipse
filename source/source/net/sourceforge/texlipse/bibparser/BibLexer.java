...
/**
 * BibTeX lexer. Extends the lexer generated by SableCC from the generation
 * file. Implements handling strings with matched braces.
 * 
 * @author Oskar Ojala
 */
public class BibLexer extends Lexer {
	...
	/**
	 * We define a filter that recognizes braced strings and removes the
	 * preamble and scribe-comments
	 */
		...
		if (state.equals(State.BRACESTRING)) {
			...
			if (stringStart == null) {
				...
				if (token instanceof TLBrace)
					...
					throw new LexerException("[" + stringStart.getLine() + "," + stringStart.getPos()
				...
				if (count != 0) {
					...
					TStringLiteral tsl = new TStringLiteral(text.toString(), stringStart.getLine(),
							stringStart.getPos());
					// emit the string
					token = tsl;
