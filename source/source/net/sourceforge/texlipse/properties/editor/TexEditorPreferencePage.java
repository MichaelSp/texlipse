 ...
 * 
 * Preferences are stored in the preference store that belongs to the main
 * plug-in class.
 * 
...
public class TexEditorPreferencePage extends FieldEditorPreferencePage implements IWorkbenchPreferencePage {
	...
	public void createFieldEditors() {
		...
		addField(new BooleanFieldEditor(TexlipseProperties.TEX_COMPLETION,
				TexlipsePlugin.getResourceString("preferenceTexCompletion"), getFieldEditorParent()));
		String completionMessage = TexlipsePlugin.getResourceString("preferenceTexCompletionDelay")
				.replaceFirst("%1", "" + MIN_COMPLETION_DELAY).replaceFirst("%2", "" + MAX_COMPLETION_DELAY);
		IntegerFieldEditor completionDelay = new IntegerFieldEditor(TexlipseProperties.TEX_COMPLETION_DELAY,
				completionMessage, getFieldEditorParent());
		...
		addField(new BooleanFieldEditor(TexlipseProperties.TEX_ITEM_COMPLETION,
				TexlipsePlugin.getResourceString("preferenceTexItemCompletion"), getFieldEditorParent()));
		...
		addField(new BooleanFieldEditor(TexlipseProperties.AUTO_PARSING,
				TexlipsePlugin.getResourceString("preferenceAutoParsing"), getFieldEditorParent()));
		String autoParsingMessage = TexlipsePlugin.getResourceString("preferenceAutoParsingDelay")
				.replaceFirst("%1", "" + MIN_AUTO_DELAY).replaceFirst("%2", "" + MAX_AUTO_DELAY);
		...
		addField(new BooleanFieldEditor(TexlipseProperties.SECTION_CHECK,
				TexlipsePlugin.getResourceString("preferenceSectionCheck"), getFieldEditorParent()));

		// Mark occurences (references and environments)
		addField(new BooleanFieldEditor(TexlipseProperties.TEX_EDITOR_ANNOTATATIONS,
				TexlipsePlugin.getResourceString("preferencesEditorHighlighting"), getFieldEditorParent()));
		...
		addField(new BooleanFieldEditor(TexlipseProperties.WORDWRAP_DEFAULT,
				TexlipsePlugin.getResourceString("preferenceWrappingDefault"), wordWrapParent));
		TexlipsePreferencePage.addSpacer(wordWrapParent);

		String message = TexlipsePlugin.getResourceString("preferenceWrapLineLength")
				.replaceFirst("%1", "" + MIN_WRAP_LENGTH).replaceFirst("%2", "" + MAX_WRAP_LENGTH);
		IntegerFieldEditor wordWrapLength = new IntegerFieldEditor(TexlipseProperties.WORDWRAP_LENGTH, message,
				wordWrapParent);
		...
		// WorkbenchHelp.setHelp(wordWrapLength.getTextControl(wordWrapParent),
		// TexlipseHelpIds.WRAP_LENGTH);
		...
		addField(new RadioGroupFieldEditor(TexlipseProperties.WORDWRAP_TYPE,
				TexlipsePlugin.getResourceString("preferenceWrapType"), 1, new String[][] {// {
						// TexlipsePlugin.getResourceString("preferenceWrapNoneLabel"),
						// TexlipseProperties.WORDWRAP_TYPE_NONE },
						{ TexlipsePlugin.getResourceString("preferenceWrapSoftLabel"),
								TexlipseProperties.WORDWRAP_TYPE_SOFT },
						{ TexlipsePlugin.getResourceString("preferenceWrapHardLabel"),
								TexlipseProperties.WORDWRAP_TYPE_HARD } }, getFieldEditorParent(group)));
	}

	/**
	 * Encapsulate group members to own components. This is necessary to get the
	 * layout right.
	 * 
	 * @param group
	 *            field editor group
