...
public class TexCollapseAction implements IEditorActionDelegate {
	...
	public void run(IAction action) {
		// get the selection and determine if we want to collapse the current
		// fold
	...
	private void collapseDeepestMatching(ProjectionAnnotationModel model, int offset) {
		...
		Iterator<TexProjectionAnnotation> iter = model.getAnnotationIterator(); iter.hasNext();) {
			TexProjectionAnnotation tpa = iter.next();
	 ...
	 *            The annotation model to use
	 * @param startOffset
	 *            The document offset of the start of the interval
	 * @param endOffset
	 *            The document offset of the end of the interval
	 */
	private void collapseAllContained(ProjectionAnnotationModel model, int startOffset, int endOffset) {
		for (@SuppressWarnings("unchecked")
		Iterator<TexProjectionAnnotation> iter = model.getAnnotationIterator(); iter.hasNext();) {
			TexProjectionAnnotation tpa = iter.next();
