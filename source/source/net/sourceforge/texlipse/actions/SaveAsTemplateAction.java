...
public class SaveAsTemplateAction implements IWorkbenchWindowActionDelegate, IEditorActionDelegate {
	...
	/**
	 * Creates a window for entering the template name, checks the user's input
	 * and proceeds to save the template.
	 */
	public void run(IAction action) {
		...
		InputQueryDialog dialog = new InputQueryDialog(editor.getEditorSite().getShell(),
				TexlipsePlugin.getResourceString("templateSaveDialogTitle"), TexlipsePlugin.getResourceString(
						"templateSaveDialogMessage").replaceAll("%s", fullname), file.getName().substring(0,
						file.getName().lastIndexOf('.')), new IInputValidator() {
		...
		if (dialog.open() == InputDialog.OK) {
			...
			if (ProjectTemplateManager.templateExists(newName)) {
				reallySave = MessageDialog.openConfirm(editor.getSite().getShell(), TexlipsePlugin
						.getResourceString("templateSaveOverwriteTitle"),
