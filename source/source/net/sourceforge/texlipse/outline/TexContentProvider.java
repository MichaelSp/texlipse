...
/**
 * Content provider for the TeX document outline. Contains the tree (list of
 * root nodes) of OutlineNodes gotten from TexDocumentModel.
 * 
...
public class TexContentProvider implements ITreeContentProvider {
	...
	public boolean hasChildren(Object element) {
		...
		if (children != null && children.size() != 0) {
			...
			for (OutlineNode n : children) {
				if (filter.select(null, element, n))
					return true;
			}
			return false;
		} else {
	 ...
	 * tree. This is only possible, if only the positions of the elements
	 * changed.
	 * 
	 * @param viewer
	 * @param newInput
	 * @return Return true, if the update was successful. If the return value is
	 *         false, the state of the ContentProvider is not in sync with the
	 *         content in newInput.
	 */
	public boolean updateElements(Viewer viewer, List<OutlineNode> newInput) {
		...
		if (stackOld.size() != stackNew.size())
			return false;
		...
		while (!stackOld.isEmpty()) {
			...
			if (o.getType() != n.getType() || !o.getName().equals(n.getName()))
				return false;
	 ...
	 * 
	 * @param viewer
	 *            the assosiated viewer
	 * @param oldInput
	 *            the old root element list
	 * @param newInput
	 *            the new root element list
	 */
