 ...
 * 
 *         The main purpose for this class is to enable better (more usable)
 *         controls when editing table entries in the table viewer
 *         (TableViewer), which, as the name suggest, is not originally designed
 *         to be a table editor. However, this class enables fast moving around
 *         between neighboring cells by using CTRL+ArrowKeys.
 */
public class TexCellListener implements KeyListener {
	...
	/**
	 * Column, which the listener listens This is here, because there seems to
	 * be no way to extract the "current column" information from the viewer...
	 */
	private int column;
	...
	public void keyPressed(KeyEvent e) {
		...
		if ((e.stateMask & (SWT.CTRL | SWT.ALT | SWT.SHIFT)) == SWT.CTRL) {
			...
			case SWT.KEYPAD_8:
				...
				if (row > 0) {
					viewer.editElement(viewer.getElementAt(row - 1), column);
					texRowList.getRows().get(row)
							.setCol(column, ((int) sum == sum) ? Integer.toString((int) sum) : Double.toString(sum));
	 ...
	 * KeyListener method from KeyListener Interface
	 * 
	 * Determines the actions to be taken, when a key is released (which is
	 * absolutely nothing).
	 * 
	 * @param e
