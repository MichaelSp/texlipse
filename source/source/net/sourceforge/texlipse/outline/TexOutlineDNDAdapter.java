...
/**
 * The Drag'n'Drop adapter for TexOutline. All the drag'n'drop operations are
 * text based.
 * 
 * Since the Positions do not seem to get updated fast enough the drag source
 * position is calculated manually.
 * 
...
public class TexOutlineDNDAdapter extends ViewerDropAdapter implements DragSourceListener {
	...
	private final TexOutlinePage outline;
	...
	/**
	 * Perform the drop. Also calculate the source remove offset before actual
	 * dropping.
	 * 
	 ...
	 * 
	 * - outline is not uptodate - transfer type is other than text - drop
	 * target is equal or children of source - target is preamble
	 * 
	 * @param target
	 * @param operation
	 * @param transferType
	...
	public boolean validateDrop(Object target, int operation, TransferData transferType) {
		...
		if (targetNode.equals(this.dragSource) || isAncestor(dragSource, targetNode)) {
			return false;
		}
	 ...
	 * 
	 * - outline is not uptodate - source is preamble
	 * 
	 * @param event
	 *            the drag event
	...
	/**
	 * Finish the drag by removing the source text if the operation was MOVE.
	 * 
