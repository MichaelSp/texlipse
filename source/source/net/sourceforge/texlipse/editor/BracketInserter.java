...
public class BracketInserter implements VerifyKeyListener, ILinkedModeListener {
	...
	/**
	 * Returns true if <i>next</i> is a character that could stand behind a
	 * closing quotation mark instead of a white space
	 * 
	 * @param next
	 * @return
	 */
	...
	/**
	 * Returns true if <i>next</i> is a character that could stand before an
	 * opening quotation mark instead of a white space
	 * 
	 * @param next
	 * @return
	 */
	...
	public void verifyKey(VerifyEvent event) {
		...
		try {
			if (document instanceof IDocumentExtension3) {
				try {
					String contentType = ((IDocumentExtension3) document).getContentType(TexEditor.TEX_PARTITIONING,
							offset, false);
			...
			if (Character.isWhitespace(next) || isClosingBracket(next)) {
				// For a dollar sign we need a whitespace before and after the
				// letter
			...
			level.fUI.setExitPolicy(new ExitPolicy(closingCharacter, (char) 0, fBracketLevelStack, sourceViewer));
