	...
	protected Control createContents(Composite parent) {
		...
		remove.addSelectionListener(new SelectionAdapter() {
			public void widgetSelected(SelectionEvent event) {
				...
				if (s >= 0) {
					boolean reallyDelete = MessageDialog.openConfirm(
							new Shell(),
							TexlipsePlugin.getResourceString("preferenceProjectTemplateConfirmTitle"),
							TexlipsePlugin.getResourceString("preferenceProjectTemplateConfirmDelete").replaceAll("%s",
									templateList.getItem(s)));
					if (reallyDelete) {
